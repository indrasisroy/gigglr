...
Issue with Venue List					
</div>
<div class="panel-body">
<div class="row">
<div class="col-md-4 col-sm-4">

<?php echo Form::open(array('url' => ADMINSEPARATOR.'/admin_issue_with_venue', 'method' => 'get','id'=>'countrylistfrmid','class'=>"form-horizontal form-border no-margin" ));?>
...
<?php echo Form::text("srch1", $value=$srch1, $attributes = array( "id"=>"srch1","class"=>" form-control input-sm ","placeholder"=>"Search by Gig Name or Booker Nickname or Venue Nickname")); ?>
...
<!--<th>Booker E-mail</th>-->
<th data-sortname="venuename" data-sorttype="<?php echo $sorttype1; ?>" class="sorttrackclass">Venue Nickname<i  class=" <?php echo $default_srt_class3; ?>"></i></th>
<!--<th>Venue Creator E-mail</th>-->
...
@if(!empty($pagi_country) && ( $pagi_country->count()>0 ))
    @foreach($pagi_country as $countries_obj)
        ...
        <tr>							
            <td>{{ stripslashes($countries_obj->gig_name) }}</td>
            ...
            <td>{{ stripslashes($countries_obj->venue_nickname) }}</td>
            <!--<td>{{ stripslashes($countries_obj->venue_creator_email) }}</td>-->
			<td>
				<?php
            ...
            <td><a href="{{ url(ADMINSEPARATOR.'/view_venue/'.$countries_obj->id) }}"><i class="fa fa-eye" title="View Details" aria-hidden="true"></i></a></td>
        </tr>
    @endforeach
@else
...
@endif
		...
		window.location.href="<?php echo url(ADMINSEPARATOR.'/admin_issue_with_venue');?>";								
		});
