...
@section('content')
	...
	if(!empty($amenitiesrow))
	{
		...
		$id=$amenitiesrow->id;
		 ...
		 $imgurldata = BASEURLPUBLICCUSTOM.'upload/amenities-image/source-file/'.$amenitiesrow->amenity_img;
...
<!-- Add fancyBox main JS and CSS files ends -->
			...
			function showIMageFancy(imagename)
			{
				...
				if(imagename!='')
				{
					$.fancybox.open("<?php echo BASEURLPUBLICCUSTOM.'upload/amenities-image/source-file';?>"+"/"+imagename);
				
					$('.fancybox-opened').css('z-index',999999);
				}
			}
//			$(document).on('blur','#amenity_name',function(){
//			  var amenity_name=$(this).val();
//			  if(amenity_name!=''){
//				 if ( /[^A-Za-z\d]/.test(amenity_name)) {
//				    $('.newerrorcustclass').html('Please enter only letter');
//    	            return (false);
//				 }
//				 if( /[^0-9]/.test(amenity_name)){
//					$('.newerrorcustclass').html('Please enter only letter');
//    	            return (false);
//				 }
//			  }
