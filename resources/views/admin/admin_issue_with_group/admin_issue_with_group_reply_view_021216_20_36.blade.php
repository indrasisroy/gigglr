...
Issue with group reply section
</div>
        ...
        <label class="control-label col-sm-6">Select recipients to reply:</label>
           ...
           {{ Form::checkbox('booker_email', $complained_by,null,$attrbar) }}
             ...
             <?php
                ...
                $attrbar['class'] = "control-label";
            ?>
            {{ Form::label('artist_email','Group email',$attrbar)}}
           <!-- Artist email-->
            <?php
                $attrbar = array();
...
<div class="panel-footer">
		...
		?>
		 <a class="btn btn-warning" href="<?php echo  url(ADMINSEPARATOR.'/view_group/'.$id);?>"><i class="fa fa-chevron-left"></i> Back</a>
		...
		<button type="button" id="disputeartistreply" data-typeid="2" class="btn btn-success btn-sm bounceIn animation-delay5  pull-right">Submit</button>
		<?php
        }?>
</div> 
<?php
			...
			if (r == true) {
				...
				var url= "<?php echo url(ADMINSEPARATOR.'/artistbookertransactionbyadmin');?>";
				    $.ajax({
                        ...
                        type: "POST",
						dataType:'json',
                        data:{'artist_type':'2','pay_to_booker_id':pay_to_booker_id,'pay_to_booker_id_value':pay_to_booker_id_value,'pay_to_artist_id':pay_to_artist_id,'pay_to_artist_id_value':pay_to_artist_id_value,'resolve_dispute_id':resolve_dispute_id,'gig_id':gig_id,'gig_unique_id':gig_unique_id,'_token':$('meta[name="csrf-token"]').attr('content')},
                        success:function(d){
						console.log(d);
						toastr.remove();
						var flag = "";
