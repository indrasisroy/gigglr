						...
						?>
							...
							<tr>
								...
								<td>
									...
									<?php echo ($verified_status > 0 ? 'Yes' : 'No') ?>
...
<script>
	...
	jQuery(document).ready(function(){
		...
		jQuery(".statustrkclass").click(function(){
			...
			if (chkconfstatus==true)
			{
				...
				if (statuschange_data==1) {
					changing_status(statuschange_data,userid_data,groupinfo_data,venueinfo_data,flagval=1);
                }
				else{
		...
		jQuery("#causebut").click(function(){
			...
			jQuery.ajax({
				...
				success: function(d)
				{
                    ...
                    {
						changing_status(statuschange_data,userid_data,groupinfo_data,venueinfo_data,flagval=0);
						$('#deactivation_cause').modal('hide');
						$('#cause').val('');
                    }
				}
		...
		function changing_status(statuschange_data,userid_data,groupinfo_data,venueinfo_data,flagval)
		{
		...
		//	alert(venueinfo_data);
			...
			var snddata = {_token:"<?php echo csrf_token(); ?>",statuschange: statuschange_data,userid:userid_data,groupinfo_data:groupinfo_data,venueinfo_data:venueinfo_data,flagval:flagval}; 
		//	alert(JSON.stringify(snddata));						  
			jQuery.ajax({
				...
				success: function(data)
				{
            		...
            		poptriggerfunc(msgtype = 'success', titledata = '', msgdata = "User Status updated successfully", sd = 1000, hd = 1500, tmo = 2000, etmo = 2000, poscls = 'toast-bottom-right');
				}
			});
		}
