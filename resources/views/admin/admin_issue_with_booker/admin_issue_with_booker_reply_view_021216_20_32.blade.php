...
<div class="panel-body" id="dispute_resolved_div">
<?php
echo Form::open(array('url' => '','method' =>'post','id'=>'formdisputeid','class'=>"form-horizontal form-border no-margin" ));?>
	...
	<div class="form-group row" id="resolve_the_dispute_div" style="display:none">
		...
		<div class="form-group">
			<label class="col-lg-6 control-label">Pay to booker</label>
				<div class="col-lg-6">
					<input class="form-control" type="text" id="pay_to_booker" data-booker="{{$pagi_country->booker_id}}" name="pay_to_booker" placeholder="input here...">
		...
		<div class="form-group">
			<label class="col-lg-6 control-label">Pay to artist</label>
				<div class="col-lg-6">
					<input class="form-control" type="text" id="pay_to_artist" data-artist="{{$complained_by}}" name="pay_to_artist" placeholder="input here...">
				</div>
		</div>
		<div class="panel-footer">
        ...
        <label class="control-label col-sm-6">Select recipients to reply:</label>
           ...
           <!-- Booker email-->
            <?php
                ...
                $attrbar['class'] = "email";
            ?>
           {{ Form::checkbox('booker_email', stripslashes($pagi_country->booker_id),null,$attrbar) }}
           ...
           <!-- Artist email-->
            <?php
                ...
                $attrbar['class'] = "email";
            ?>    
            {{ Form::checkbox('artist_email', $complained_by,null,$attrbar) }}  
...
<div class="panel-footer">
		...
		?>
		<a class="btn btn-warning" href="<?php echo  url(ADMINSEPARATOR.'/view_booker/'.$id);?>"><i class="fa fa-chevron-left"></i> Back</a>
		...
		<button type="button" id="disputeartistreply" data-typeid="4" class="btn btn-success btn-sm bounceIn animation-delay5  pull-right">Submit</button>
		<?php
        }?>
...
$(document).ready(function () {
        ...
        $(document).on('click','#disputeartistreply',function(){
            ...
            if((jQuery.type(artist_ID) === "undefined") && (jQuery.type(booker_ID) === "undefined")){
                ...
                var snddata = {_token:"<?php echo csrf_token(); ?>",description:description_field,resolve_dispute_id:resolve_dispute_id,gig_id:gig_id,gig_unique_id:gig_unique_id,booker_id:booker_id,artist_id:artist_id,booker_ID:booker_ID,artist_ID:artist_ID,last_row:last_row,user_type:user_type};
				var respdata;
                console.log(snddata);
		...
		$('#paybutton').click(function(){
			...
			var total_amount = {{ $pagi_country->total_amount }};
			...
			if (r == true) {
				...
				var url= "<?php echo url(ADMINSEPARATOR.'/artistbookertransactionbyadmin');?>";
				    $.ajax({
                        ...
                        type: "POST",
						dataType:'json',
                        data:{'artist_type':'1','pay_to_booker_id':pay_to_booker_id,'pay_to_booker_id_value':pay_to_booker_id_value,'pay_to_artist_id':pay_to_artist_id,'pay_to_artist_id_value':pay_to_artist_id_value,'resolve_dispute_id':resolve_dispute_id,'gig_id':gig_id,'gig_unique_id':gig_unique_id,'_token':$('meta[name="csrf-token"]').attr('content')},
                        success:function(d){
						toastr.remove();
						var flag = "";
						if (d.returnflag == '1') {
