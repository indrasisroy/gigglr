            ...
            <div class="padding-md">
                <div class="row">
                    ...
                    <div class="panel-heading">
                        Email Template List
                        <span class="pull-right"><a type="button" class="btn btn-xs btn-success" href="<?php echo  url(ADMINSEPARATOR.'/createemailtemplate'); ?>" >ADD</a></span>
                    </div>
                    <div class="panel-body">
                        <div class="row">
                            <div class="col-md-4 col-sm-4">
                            <?php echo Form::open(array('url' => ADMINSEPARATOR.'/email-template', 'method' => 'GET','id'=>'emailearchfrmid','class'=>"form-horizontal form-border no-margin" )); ?>
                                ...
                                if( !empty($useinPagiAr) && array_key_exists('sort1',$useinPagiAr))
                                {
                                 ...
                                 <?php echo Form::text("srch1", $value=$srch1, $attributes = array( "id"=>"srch1","class"=>" form-control input-sm ","placeholder"=>"search by subject")); ?>
                    ...
                    <table class="table table-striped" id="responsiveTable">
                        <thead>
                            <tr>
                                <th data-sortname="subject" data-sorttype="<?php echo $sorttype1; ?>" class="sorttrackclass" >Subject <i  class=" <?php echo $default_srt_class1; ?>"></i></th>
                       ...
                       <?php
                        ...
                        if(!empty($pagi_email) && ( $pagi_email->count()>0 ) )
                        {
                        
                                foreach($pagi_email as $email_obj)
                                {
                                    $art_connctId = $email_obj->id;
                                    $status = $email_obj->status;
                                    $status_change_to=($status==1)?0:1;
                                    $status_icon_class=($status==1)? "fa fa-unlock fa-lg":"fa fa-lock fa-lg";


                                    $emailtemplatesubject = $email_obj->subject;//******Email Template Subject
                                    $email_subjectlen = strlen($emailtemplatesubject); //***** Email Template Subject Length

                                     if($email_subjectlen > 25)
                                    {
                                        $emailtemplatesubject = substr($emailtemplatesubject, 0,25).'..';
                                    }

                        ?>
                            <tr>
                                
                                <td><?php echo stripslashes($emailtemplatesubject); ?></td>
                              <!--   <td><?php// echo stripslashes($email_obj->message); ?></td> -->
                    
                                <td>
                                <a href="<?php echo  url(ADMINSEPARATOR.'/createemailtemplate/'.$art_connctId); ?>"><i class="fa fa-edit fa-lg"></i></a>
                             
                               <!--  <a href="javascript:void(0);" ><i id="stat_<?php// echo $email_obj->id; ?>" href="javascript:void(0);" data-emailstsid="<?php// echo $email_obj->id; ?>" data-statuschange="<?php// echo $status_change_to; ?>" class="statustrkclass <?php// echo $status_icon_class; ?>"></i></a>  -->
                               
                                <!-- <a href="<?php// echo  url(ADMINSEPARATOR.'/delete_template/'.$art_connctId); ?>" onclick="javascript:return confirm('Are you  sure you want to delete?')"><i class="fa fa-trash-o fa-lg"></i></a> -->
                                </td>
                    ...
                    <?php echo $pagi_email->appends($useinPagiAr)->render(); ?>
                     </div>
                </div><!-- /panel -->
            ...
            <script>
                ...
                jQuery("#srchbutton").click(function() //**********Blank Submission Starts here
                {
                    ...
                    if (srch1data=='')
                    {
                                ...
                                window.location.href="<?php echo url(ADMINSEPARATOR.'/email-template');?>";
                    }
                    else
                    {
                                 
                                jQuery("#emailearchfrmid").submit();
                    }
                ...
                var emailid_data=jQuery(this).data('emailstsid');
                
                if (chkconfstatus==true)
                {
                
                                var snddata = {_token:"<?php echo csrf_token(); ?>",statuschange: statuschange_data,emailid:emailid_data};
                                ...
                                jQuery.ajax({
                                                type: "POST",
                                                data:snddata,
                                                url: "<?php echo url(ADMINSEPARATOR.'/emailchangestatus');?>",
                                                dataType:"json",
