   ...
   function getfilterevtypedata()
   {
        var filterevtypedata=filterevtype;
        return filterevtypedata;
   }
    ...
    function getmyprocategorydata()
	{
			...
			var pro_category_data=pro_category;
			//console.log("=newselecteddate=>"+newselecteddate);
			return pro_category_data;
	}
    
    function getmyprogenredata()
	{
		
			var pro_genre_data=pro_genre;
			//console.log("=newselecteddate=>"+newselecteddate);
			return pro_genre_data;
	}
    
    function gettypeflagdata()
    {
        var type_flag_data=type_flag;
        
        return type_flag_data;
    }
	...
	function renderCustCalendarRoster(newdefdt)
		{
				...
				events: {
							url: base_url_data+'/publicprofilegigs',
							type: 'POST',
							...
							return {
                                        ...
                                        seo_name:seo_name,
                                        type_flag:gettypeflagdata(),
                                        category:getmyprocategorydata(),
                                        genre:getmyprogenredata(),
                                        filterevtype:getfilterevtypedata()
			...
			dayClick: function(date, jsEvent, view)
			{
				...
				if (chk.length==0 && (selmnthdata==calendarcurmnth) )
				{
                        ...
                        //**** call left menu populate
                         cal_select_date=selecteddate;
                         
                         //loadprofileleftmenu();
                         //console.log(cal_select_date+"***"+seo_name+"******"+type_flag);
                         loadprofileleftmenu();
			...
			eventAfterRender: function(event, el)
			{
                        ...
                        // console.log("'111====> data-startdatetime='"+event.start.format("YYYY-MM-DD HH:mm:ss")+"  data-enddatetime='"+event.event_end.format("YYYY-MM-DD HH:mm:ss")); 
                        //var enddatetime=event.end.format("YYYY-MM-DD"); // currently not using because formatting  in calendar getting changed for end date on different date from satrt date 
                        var enddatetime=event.evcustomend;
                       // console.log("'222====> data-startdatetime='"+event.start.format("YYYY-MM-DD HH:mm:ss")+"  data-enddatetime='"+event.event_end.format("YYYY-MM-DD HH:mm:ss")); 
                        ...
                        else if (rcsipsbb_data=='BLACKSTAR')
                        {
