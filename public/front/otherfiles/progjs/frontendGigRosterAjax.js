...
$.confirm({
                                    ...
                                    title: false,
                                    content: "<p style='text-align:center;'>Are you sure <br> You want to "+decline_text+" this offer?</p>",
                                    closeIcon: true,
...
$.confirm({
                                    ...
                                    title: false,
                                    content: "<p style='text-align:center;'>Are you sure you want to accept this Offer?</p>",
                                    closeIcon: true,
                                    closeIconClass: 'fa fa-close',
                       ...
                       success:function(d){
                        ...
                        if (d.flag==0 && d.msg!='')
                        {
                           ...
                           $("#accept_bid_reqst_booker_artst").prop("disabled",false);
                            ...
                            poptriggerfunc(msgtype='error',titledata='',msgdata=d.msg,sd=1000,hd=1500,tmo=3000,etmo=3000,poscls='toast-top-full-width');          
                            removeallnegotiationmodal();
                          ...
                          }else if(d.flag==999 && d.msg!='')
                          {
                            //******************  condition fo opening modal starts here
                            ...
                            //poptriggerfunc(msgtype='error',titledata='',msgdata=d.msg,sd=6000,hd=1500,tmo=3000,etmo=4000,poscls='toast-top-full-width');          
                           // removeallnegotiationmodal();
                            paymodalopenednormal=false;
                            $('#myModal3paymentview').modal('show');
                            $('#cc-amount').val(d.amountneed);


                            //******************  condition fo opening modal ends here
                          }
                        else{ 
                                $("#processcls_negoall").addClass('mydisplaynone');
                                ...
                                // console.log("amount val ==> "+parseFloat(d.amount_have).toFixed(2));


                                //**************** starts here

                                  if(d.amount_have != 999990975)
                                  {
                                    $("#wallet_amount_id").html("You have $"+ parseFloat(d.amount_have).toFixed(2));
                                  }
...
//*****************  Accecpt bid request ends here
  ...
  function validatenegotiationaccept(submittrueorflase,submittotalamountval,aggreeval)
  {
   ...
   else if(aggreeval == 'za')
   {
     $("#i_agree_id").parent().addClass('radioerrorcolor');
     flagval = 0;
   }else if(t1 == false && cancellation_fee !="")
   {
     $("#cancellation_fee").parent().addClass('errorField');
     flagval = 0;
   }
   else if(t2 == false && security_payment_gig!="")
   {
   ...
   else if(t == true)
   {
    ...
    if(cancellation_fee == "")
      ...
      if(security_payment_gig == "")
      {
        security_payment_gig = '0.00';
      }else if(t2 == false)
      {
        security_payment_gig = '0.00';
      }

     var ff2 =parseFloat(submittotalamountval)-(parseFloat(cancellation_fee)+parseFloat(security_payment_gig));
     var f = parseFloat(ff2).toFixed(2);
    // console.log(f);
     if(f < 0)
     {
...
$("#security_payment_gig").keypress(function()
{
  $("#security_payment_gig").parent().removeClass('errorField');
})
