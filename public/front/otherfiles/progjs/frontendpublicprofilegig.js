          ...
          var cmsgdata='data updated successfully';
             ...
             $.ajax({
                 ...
                 success:function(d){
                    ...
                    $("#profileCatFen").html(ep_contents).fadeIn(10,function(){
                        

                        //***** bind change starts
                        $("#category_sub_profile").selectpicker('refresh');
...
$('.allprofileeventcls').click(function(evnt){
    ...
    if (evtypeflagdata==1)
         {
                var filterevtypedta = $(this).data('evtype');
                
              
             
             
             
                //******* here for add or minus symbol on  tabs now starts ***************
             
               
             
                
                var pendingbkshowflag_data= $(this).data('pendingbkshowflag');
             
                    if(pendingbkshowflag_data==1)
                    {
                        // reassign  pendingbkshowflag_data to 0
                        
                        $(this).data('pendingbkshowflag',0);
                        // add_icon remove_icon
                        $(this).find(".pendbkpblshowflicocls").removeClass("remove_icon");
                        $(this).find(".pendbkpblshowflicocls").addClass("add_icon");
                        
                    }
                    else if(pendingbkshowflag_data==0)
                    {
                            // reassign  pendingbkshowflag_data to 1
                        
                            $(this).data('pendingbkshowflag',1);
                        
                            // add_icon remove_icon
                            $(this).find(".pendbkpblshowflicocls").removeClass("add_icon");
                            $(this).find(".pendbkpblshowflicocls").addClass("remove_icon");
                    }
             
             
                    //*** check evnttypeshowflag to   delete  in  array starts
             
                  //console.log("=filterevtypedta=>"+filterevtypedta);
                 // console.log("= previous evnttypeshowflagAr=>"+evnttypeshowflagAr.toString());
               ...
               // console.log("= Now evnttypeshowflagAr=>"+evnttypeshowflagAr.toString());
                ...
                //******* here for add or minus symbol on  tabs now ends ***************  
         ...
         else
         {
            ...
            toastr.remove();
            poptriggerfunc(msgtype='error',titledata='',msgdata="InActive  feature",sd=6000,hd=1500,tmo=3000,etmo=4000,poscls='toast-top-full-width'); 
            evnt.preventDefault();
             ...
             $.ajax({
                 ...
                 success:function(d)
                 {
                    if (d.return_type == '1') {
                        ...
                        $(".loadLeftGigList").html("<span class='nodatacustlftclass'>No events to display</span>");
                        //poptriggerfunc(msgtype='error',titledata='',msgdata="Record not found",sd=6000,hd=1500,tmo=3000,etmo=4000,poscls='toast-top-full-width');         
                    }
