...
//******* reset  payment  credit card form starts *******************************
      ...
      function loadtransactiondatas (pagenumdata,pgloadflag,searchdata)   
           {
                ...
                if(typeof(local_tz_data)== 'undefined')
                 ...
                 var callurlwithdata={_token:csrf_token_data,pagenum:pagenumdata,local_tz_data:local_tz_data};
 ...
 <tr class="total"><td>&nbsp;</td><td><span>Total</span></td><td><span class="lett-spac">$11900</span></td><td><span class="lett-spac">$2100</span></td></tr>
    ...
    function chargepinpayment(card_token)
      {
            $(".ccp_process_cls").removeClass("mydisplaynone");  
                        ...
                        if(success_data=="1" && (message_data=='') )
                            {
                                 ...
                                 poptriggerfunc(msgtype='success',titledata='',msgdata="Thank you. Your payment was successful",sd=1000,hd=1500,tmo=3000,etmo=4000,poscls='toast-top-full-width');
                                  ...
                                  $(".ccp_process_cls").addClass("mydisplaynone"); 
                                
                                if(paymodalopenednormal==false)
                                    {
                                        paymodalopenednormal=true; //** reassign it to true 
                                        ...
                                        $("#myModal3paymentview").modal('hide'); //**** hide payment modal 
 ...
 //********************** wallet payment related code starts ***************************       
  ...
  form.submit(function(e) {
    ...
    // Fetch details required for the createToken call to Pin Payments
      ...
      var ccnumberdata = $('#cc-number').val();
      //console.log("Prev=>"+ccnumberdata);
      ccnumberdata=ccnumberdata.replace(/\s+/g, '');
      //console.log("Now=>"+ccnumberdata);
    ...
    $('a[data-toggle="tab"]').on('shown.bs.tab', function (e) {
        ...
        if(paytabtargetiddata=="#paythree")
            ...
            else if(paytabtargetiddata=="#payone")
            {
                    ...
                    $("#bankpaymentsubmid").attr({disabled: true});    
                    ...
                    if(allowbankpaymntflag==false)
                        {
                            ...
                            //                            poptriggerfunc(msgtype='error',titledata='',msgdata=validation_err_msg,sd=1000,hd=1500,tmo=3000,etmo=4000,poscls='toast-top-full-width');
                              setTimeout(function(){
                                    ...
                                    $(".bt_process_cls").addClass("mydisplaynone");  
                                   $("#bankpaymentsubmid").removeAttr('disabled'); 
                    ...
                    $(".bt_process_cls").removeClass("mydisplaynone");    
           ...
           $('#myModal3paymentview').on('shown.bs.modal', function () {
            ...
            //console.log("payment modal opened");
               ...
               //*** new code starts
                ...
                $('#cc-number').unbind('keyup');
                $('#cc-number').bind('keyup', function (e) {

                var kcodedata = e.keyCode || e.which;
                    kcodedata=parseInt(kcodedata);
                    
                var kcodenotallowedAr=[];
                    kcodenotallowedAr.push(8);
                    kcodenotallowedAr.push(46);
                   // kcodenotallowedAr.indexOf(kcodedata);
                    
                    var chkpresent=kcodenotallowedAr.indexOf(kcodedata);
                    var ccnmlen= $('#cc-number').val().length;
                    //console.log(kcodedata);
                    
                    if(chkpresent==-1 && ccnmlen<19)
                    {
                        //console.log('entered');
                        $(this).val(function (index, value) {
                            
                        //return value.replace(/\W/gi, '').replace(/(.{4})/g, '$1 ');
                            return value.replace(/\W/gi, '').replace(/(.{4})/g, function myFunction(x){
                                  //console.log(x+' ');
                                return x+' ';
