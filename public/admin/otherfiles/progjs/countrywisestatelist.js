...
function deleteusrgroupimg()
{
 ...
 //*************script for send activation link starts here

            $("#activationlinkresend").click(function(){


                if (confirm('Are you sure you want to send activation link?')) {
    // Save it!


               var emailval = $(this).data('id');
               var idemailvalsplit;
               idemailvalsplit=emailval.split('#####');
               var clurl = admin_base_url_data + '/sendactivationlinkfromadmin';

                    var sndata = {
                        _token: admin_csrf_token_data,
                        userID: idemailvalsplit[1],
                        useremail: idemailvalsplit[0],
                    ...
                    //************ ajax content starts here
                            ...
                            success: function(res) {
                                 toastr.remove(); // Immediately remove current toasts without using animation
                                 poptriggerfunc(msgtype = 'success', titledata = '', msgdata = "Account activation email sent successfully", sd = 1000, hd = 1500, tmo = 2000, etmo = 2000, poscls = 'toast-bottom-right');
    ...
    // Do nothing!
                    ...
                    //************ ajax content ends here

                   // alert(" sndata userID  "+sndata.userID+" ===== sndatauseremail  "+sndata.useremail);

                

            });

            //************ script for send activation link ends here
