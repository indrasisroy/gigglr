...
$LatLongArray['latlong'] = $AllLatLongArray;
 ...
 //**** for reverse geo coding  finding city  rom lat long starts
  ...
  $getLatlong = file_get_contents("https://maps.googleapis.com/maps/api/geocode/json?latlng=22.5726500,88.3638900&key=AIzaSyDMtfReOBcDyXEUBByLlLgjOkwMKKJw86M");
  //echo $getLatlong;
 ...
 //**** for reverse geo coding  finding city  rom lat long ends
   ...
   function getTimezone($lat='',$lng='')
   {
       ...
       // if($lat!= '' && $lng!='')
       // { 
       //     $gettimezone =file_get_contents('https://maps.googleapis.com/maps/api/timezone/json?location=+'.$lat.','.$lng.'+&timestamp=1331766000&key=AIzaSyDTX-LaXc4n4UWAU6mn7HlN76MmBjq55jk');
       ...
       //        else
       //        {
       //             $Timezonearray = array("flag" => 0,"timeZoneId" => '', "timeZoneName" => 'Australia/Sydney');
       ...
       //      $Timezonearray = array("flag" => 0,"timeZoneId" => 'Australia/Sydney', "timeZoneName" => 'Australian Eastern Time');
...
function paginatecustom($reload, $page, $tpages,$ajaxstatus) {
  ...
  //**** for ajax pagination ends
    ...
    $settingsgoogleapi = $settingsdatares->google_api_key ? $settingsdatares->google_api_key : '';
     ...
     $goople_distance_api = "https://maps.googleapis.com/maps/api/distancematrix/json?origins=".$fAddress."&destinations=".$sAddress."&mode=driving&language=en-EN&key=AIzaSyDTX-LaXc4n4UWAU6mn7HlN76MmBjq55jk";
    ...
    function convertdatetothistz($dttm='',$ftmzn='UTC',$ttmzn='UTC',$cnvrtdtdrmt='Y-m-d H:i:s')
    {
            $respar=array();
            $time=$dttm;
            $fromTz=$ftmzn;
			...
			$convdttime= $date->format($cnvrtdtdrmt);
        
            if(!empty($convdttime))
            {
                $respar['converteddatetime']=$convdttime;
            }
			
            $respar['reqdatetime']=$dttm;
            $respar['fromtimezone']=$ftmzn;
            $respar['totimezone']=$ttmzn;
