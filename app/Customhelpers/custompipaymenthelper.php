...
**/
function check_charges_status($refund_token, $payment_type, $secret_key)
{
	//Charges details using GET Method
        $charges_url = 'https://test-api.pin.net.au/1/refunds/'.$refund_token;   // Test Payment Url
	if($payment_type == 'live')
	{
		$charges_url = 'https://api.pin.net.au/1/refunds/'.$refund_token;   // Live Payment Url
	}
	...
	//return json_decode($charges_output,true);
    
    $charges_output=json_decode($charges_output,true);
    
   // echo "<pre>charges_output=>"; print_r($charges_output); echo "</pre>";
    ...
    $flagresp=0;
        ...
        if(!empty($charges_output))
        {
            if(array_key_exists('response',$charges_output) && !empty($charges_output['response']) )
            {
                $responsear=$charges_output['response'];

                
                
                $success=$responsear['success'];
                $status_message=strtolower($responsear['status_message']); //pending, success, cancelled
                $amount=$responsear['amount']; // e.g amount charged
                $currency=$responsear['currency']; // e.g USD/AUD etc
                $refund_token=$responsear['token']; // e.g ch_sdsds3232 charge token that was used              
                $charge_token=$responsear['charge'];
                ...
                $succrespdataar['amount']=$amount;
                $succrespdataar['currency']=$currency;
                $succrespdataar['charge_token']=$charge_token;
                $succrespdataar['refund_token']=$refund_token;
                ...
                elseif($status_message=="pending")
                {
                    $flagresp=2; // pending
                }
                elseif($status_message=="cancelled")
                {
                    $flagresp=3; // cancelled
                }
                


            }
            elseif(array_key_exists('error',$charges_output))
            {
               
               
               // echo "<pre>"; print_r($respfromgateway);

                if( array_key_exists('messages',$charges_output) && !empty($charges_output['messages']))
                {
                    $responseerrmessgar=$charges_output['messages'];
                ...
                elseif( array_key_exists('error_description',$charges_output) &&  !empty($charges_output['error_description']))
                    {
                        
                       
                        $ermessagear[]=$charges_output['error_description'];
                    }
...
**/
function create_refund($charge_token, $payment_type, $secret_key,$postparamar)
{
	//Create Refund POST For Charges Token

	$refunds_url = 'https://test-api.pin.net.au/1/charges/'.$charge_token.'/refunds';  // Test payment url
	
        if($payment_type == 'live')
	{
		$refunds_url = 'https://api.pin.net.au/1/charges/'.$charge_token.'/refunds';	   // Live payment url
	}
	

     $fields_string='';
    
    if(!empty($postparamar))
    {
        foreach($postparamar as $key=>$value) { $fields_string .= $key.'='.$value.'&'; }
        rtrim($fields_string, '&');
    }
    ...
    if(!empty($postparamar))
    {
         ...
         curl_setopt($refunds_ch,CURLOPT_POSTFIELDS, $fields_string);
    }
	...
	//return json_decode($refunds_output);
      $refunds_output=json_decode($refunds_output,true);
        
        //echo "<pre>"; print_r($refunds_output); echo "</pre>";        
        ...
        if(!empty($refunds_output))
        {
            if(array_key_exists('response',$refunds_output) && !empty($refunds_output['response']) )
            {
                $responsear=$refunds_output['response'];
                ...
                $amount=$responsear['amount']; // e.g amount charged
                $currency=$responsear['currency']; // e.g USD/AUD etc
                $charge_token=$responsear['charge']; // e.g ch_sdsds3232 charge token that was used
               

                $succrespdataar['refund_token']=$token;
                $succrespdataar['success']=$success;
                $succrespdataar['status_message']=$status_message;
                $succrespdataar['amount']=$amount;
                $succrespdataar['currency']=$currency;
            ...
            elseif(array_key_exists('error',$refunds_output))
            {
               
               
               // echo "<pre>"; print_r($respfromgateway);

                if( array_key_exists('messages',$refunds_output) && !empty($refunds_output['messages']))
                {
                    $responseerrmessgar=$refunds_output['messages'];
                ...
                elseif( array_key_exists('error_description',$refunds_output) &&  !empty($refunds_output['error_description']))
                    {
                        
                       
                        $ermessagear[]=$refunds_output['error_description'];
                    }
