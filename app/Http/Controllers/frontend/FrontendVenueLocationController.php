...
class FrontendVenueLocationController extends Controller
{
    ...
    public function index(Request $request)
    {
           ...
           $statetypeidAr=array();
            ...
            //*****************************************  get settings data for venue location starts here **************
            $settingsqryltllng = DB::table('settings')->where('id',1)->first();
            $settingsgoogleapi = $settingsqryltllng->google_api_key ? $settingsqryltllng->google_api_key : '';
            $data['settingsgoogleapi']=$settingsgoogleapi; 
       ...
       {    // }
               ...
               // $country_result = $country_qry;
                        ...
                        if(!empty($state_db))
                        {
                                ...
                                {
                                        $stateIDAr[$state_db_obj->id]=stripslashes($state_db_obj->state_3_code);
                                }
    ...
    public function checksaveusername($request,$id=0)
           {
                ...
                "nickname" => "required|min:5|max:50"         
                  ...
                  // "nickname.required" => " Venue name is required",
                    'nickname.required'=>'*A venue title or name is required',
          ...
          public function saveuserdescvenue(Request $request)
        {
            ...
            $user_description = e($request->input('user_description',''));
    ...
    public function checksaveuserdesc($request,$id=0)
           {
                ...
                $controlmsg="";
                    ...
                    "user_description" => "required|max:1500"
                       ...
                       "user_description.required" => " A profile description is required",                  
          ...
          public function deletemyskillvenue(Request $request)
        {
         ...
         // echo "<pre>"; print_r($error_msgAr);echo "</pre>";
            ...
            if($flag_id == 3){
               ...
               $responseAr['error_message']="This genre can not be deleted";
      ...
      //************************** save ABN data starts
            ...
            {   
                        $validator = Validator::make($request->all(), [
                                    ...
                                    "abndata.required" => "Your ABN is required",
                                    "abndata.numeric" => "An ABN can only be up to 11 numbers",//"Only numerics are allowed",
                        ]);
       ...
       //************************** save GST data starts
            ...
            {   
                        $validator = Validator::make($request->all(), [
                                    ...
                                    "gstdata.required" => "To charge GST you must provide your Australian Tax File Number",
                                    "gstdata.numeric" => "Only numbers are accepted in this field",
                        ]);
            ...
            public function checksavepagemetatag($request,$id=0)
            {
                        $validator = Validator::make($request->all(), [
                                    ...
                                    "pagemetatagdata.required" => "A URL is required for your page", 
...
if($chkvalid===true)
{
                   ...
                   //$hlprdta = urlencode($addressvenue1.' '.$addressvenue2.' '.$towndata.' '.$zipdata.' '.$statename.' '.$countryname);
                       ...
                       if($addressvenue2!=''){
           $hlprdta = $addressvenue1."+".$addressvenue2."+".$towndata."+".$statename."+".$zipdata."+".$countryname;
       }else{
           $hlprdta = $addressvenue1."+".$towndata."+".$statename."+".$zipdata."+".$countryname;
       }
                   ...
                   // echo "<pre>";
                   // print_r($LatLongchk);
                  $flagvallatlong =  $LatLongchk['flag'];
                         ...
                         // echo "<br>";
                            ...
                            if($amnty>0)
                           {
                            ...
                            }else
                            {
                              ...
                              $respAr['errormsgdata']= "Please enter your legitimate address";
                              $errorflagmsg = 3;
                            }
          ...
          //echo $stateres;
           ...
           $statetypeidAr=array();
            ...
            if(!empty($stateres))
            {
                    foreach($stateres as $stateres)
                    {
                            $statetypeidAr[]=array('id'=>$stateres->id,'name'=>stripslashes($stateres->state_3_code));
                    }
...
// die;
     ...
     $end_timedata = $endtimedt.' '.$endtimedtval;

     // echo " end_timedata ======".$endtimedtval;
     // echo "<br>";

      $request->input('enddatetime');
      $request->input('endtimedt');
      $onlyendtime = $request->input('end_timedata');

      $endtimehr_val = $request->input('endtimehr_val');
      $endtimemin_val = $request->input('endtimemin_val');


      $enddatetimeformatchange = date('H:i:s',strtotime($onlyendtime));
...
//echo date('Y-m-d H:i:s',strtotime('+ 1 hour +20 minutes',strtotime($startrrrrrrrrrr))); die;
    ...
    // echo "end_timedata ".$end_timedata;
   ...
   //    echo "<br>";
    ...
    $sess_id = $request->session()->get('front_id_sess');
         ...
         //***** currency insert starts here


                                $bookercurrencyval='';
                                $bookercurrencycqry = DB::table('location_country')
                                ->select('location_country.currency_code')
                                ->where('location_country.id',$booker_country_code)
                                ->first();

                                if(!empty($bookercurrencycqry))
                                {
                                $bookercurrencyval = $bookercurrencycqry->currency_code;
                                }

          //***** currency insert ends here
                                
        //*******  get server timezone strats here
          $servertimezn=date_default_timezone_get();
        //*******  get server timezone ends here



           //********************  timezone calculation starts here *****************

                      $pretimezn = $bkngdate." ".$strttim;
                      $evntdtcnv= convertdatetothistz($dttm=$pretimezn,$ftmzn=$timezoneId,$ttmzn=$servertimezn,$cnvrtdtdrmt='Y-m-d H:i:s');
                      if(array_key_exists('converteddatetime',$evntdtcnv))
                      {
                      $event_start_ptz_date=$evntdtcnv['converteddatetime']; 
                      }
                      else
                      {
                      $event_start_ptz_date=date("F j, Y, g:i a",strtotime($pretimezn));
                      }
                      // echo $event_start_ptz_date; //die;
                      ...
                      $posttimezn = $endtimedt." ".$enddatetimeformatchange;
                      $evntdtendcnv= convertdatetothistz($dttm=$posttimezn,$ftmzn=$timezoneId,$ttmzn=$servertimezn,$cnvrtdtdrmt='Y-m-d H:i:s');
                      if(array_key_exists('converteddatetime',$evntdtendcnv))
                      {
                      $event_end_ptz_date=$evntdtendcnv['converteddatetime']; 
                      }
                      else
                      {
                      $event_end_ptz_date=date("F j, Y, g:i a",strtotime($posttimezn));
                      }
                      // echo $event_start_ptz_date; //die;
      ...
      'gigpostrequestflag' => 2,
        ...
        'event_end_date_time'=>$endtimedt." ".$enddatetimeformatchange,//$bkngdate." ".$endtim,
        'event_end_date' =>$endtimedt,
        'event_end_time'=>$enddatetimeformatchange,
        ...
        'event_timezone' => $timezoneId,
         ...
         'bk_currency_country_id' => $bookercurrencyval,
         'bk_req_date_tz' => $servertimezn,
          'event_start_date_time_ptz' => $event_start_ptz_date,
          'event_end_date_time_ptz' => $event_end_ptz_date,
        ],
...
//************************** save booking options data starts
    ...
    //         {
            ...
            public function chkvalidrules($hourly_rate,$security_deposit)
            {
                        ...
                        if ($security_deposit > $hourly_rate && $hourly_rate!='0.00')
                        //if($hourly_rate<=0)
                        {
                                    $errorMsg[]="The Security Deposit should only be a portion of the Total Amount";
                        }
            ...
            //************************** save booking options data ends
             ...
             public function commonn_email($receiver_email,$receiver_name,$flag,$eventidtoemail,$bookernm)
      {
            //echo "receiver_name=>".$receiver_name;die;
                    ...
                    if($flag == 1)
                  {
                    ...
                    $replaceto =array(ucfirst(stripslashes($receiver_name)),ucfirst(stripslashes($receiver_name)),$sitename,$copyright_year,$bsurl,$logoIMG);
                    mailsnd($Temid=14,$replacefrom,$replaceto,$receiver_email);
                  }else if($flag == 2)
                  {
                    $replacefrom =array('{Accountfname}','{User}','{SITENAME}','{YEAR}','{BASE_URL}',"{LOGO_IMG}","{EVENTID}");
                    $replaceto =array(ucfirst(stripslashes($receiver_name)),ucfirst(stripslashes($receiver_name)),$sitename,$copyright_year,$bsurl,$logoIMG,$eventidtoemail);
         ...
         public function booking_email($artistvenuegroupfirstname,$artistvenuegrouplastname,$artistvenuegroupusrname,$artistvenuegroupemail,$bookerusrname,$bookingcategory_name,$bookinggenre_name,$address1val,$address2val,$towndata,$statename,$zipdata,$countryname,$bkngdate,$bkngexpirdate,$expirtim,$eventidtoemail,$venuenickname)
      {
            //echo "receiver_name=>".$receiver_name;die;
                    ...
                    $replaceto =array(ucfirst(stripslashes($artistvenuegroupfirstname)),ucfirst(stripslashes($artistvenuegrouplastname)),'Venue',ucfirst(stripslashes($venuenickname)),ucfirst(stripslashes($bookerusrname)),$bookingcategory_name,$bookinggenre_name,$address1val,$address2val,$towndata,$statename,$zipdata,$countryname,$bkngdate,$sitename,$copyright_year,$bsurl,$logoIMG,$bkngexpirdate,$expirtim,$eventidtoemail);
       ...
       public function checksavevenueaddressamenities($request){
        ...
        public function checksavebookingrequest_venue($request,$timezoneId){
                       ...
                       $userData=array();
                        ...
                        $validator->after(function($validator)  use ($userData)  { 
                                    ...
                                    $sess_id = $request->session()->get('front_id_sess');
                                     ...
                                     $endtimemin_val = addslashes($request->input('endtimemin_val'));
                                     $endtimehr_val = addslashes($request->input('endtimehr_val'));
       
                                    if($booking_datedata!='' && $start_timedata!='' && $artist_ID!='' && $sess_id!='')
                                    {    
                                                $validatehourlyavaiability=$this->chkvalidavailability($booking_datedata,$start_timedata,$artist_ID,$sess_id,$end_timedata,$endtimehr_val,$endtimemin_val);
            ...
            public function chkvalidavailability($booking_datedata,$start_timedata,$artist_ID,$sess_id,$end_timedata,$endtimehr_val,$endtimemin_val)
            {
                                        $ttt= explode("/",$booking_datedata);
                                        $bkngdate = $ttt[2].'-'.$ttt[1].'-'.$ttt[0];
...
// echo $end_timedata;die;
            //;
                                        //echo "end_timedata = ".$end_timedata;
                                      //  echo $endtimehr_val.":".$endtimemin_val;
                                    //  $exv = explode(':',$end_timedata);

                                      // $nwdt =  date('Y-m-d H:i:s',strtotime('+ '.$exv[0].' hour +'.$exv[1].' minutes',strtotime($bookingdatetime_chk))); 
                                        $nwdt =  date('Y-m-d H:i:s',strtotime('+ '.$endtimehr_val.' hours +'.$endtimemin_val.' minutes',strtotime($bookingdatetime_chk))); 
                                       // echo " ====== ".$nwdt." ==== ";
          ...
          OR (gigm.event_start_date_time >= '".$bookingdatetime_chk."' AND gigm.event_start_date_time <= '".$nwdt."' AND gigm.event_end_date_time >= '".$bookingdatetime_chk."')
             ...
             // print_r($daily_cal_qry_data);
              ...
              // die;
                     ...
                     //********commented on 12-12-2016 starts
             ...
             public function chkduplicacy($booking_datedata,$start_timedata,$artist_ID,$sess_id,$end_timedata)
            {
              ...
              // print_r($match_date_qry);
                                              ...
                                              if ($count_bookingavailable > 0)
                                              {
                                              // $errorMsg[]="Your booking request can not be processed <br> You already booked this venue <br> Please select another date time";
                                              }
            ...
            //*******************  added for venue booking form split strats here


                public function checkduplicatedatafuncvenue(Request $request)
            {

              $respAr =array(); $venueqrycount=0; $artistqrycount=0;$groupqrycount=0;
              $bookingdataval = $request->input('bookingdateval');
              $starttime = $request->input('start_timedata');
              $artistid  =$request->input('artistid');
              $sess_id = $request->session()->get('front_id_sess');
                
              $endtimehrval = $request->input('endtimevaluehr');
              $endtimevaluemnt = $request->input('endtimevaluemnt');
              ...
              //*******
               ...
               $evntenddatetime =  date('Y-m-d H:i:s',strtotime('+'.$endtimehrval.' hour +'.$endtimevaluemnt.' minutes',strtotime($startdatetimeval)));

              // echo " startdatetimeval ".$startdatetimeval;
              // echo " sess_id ".$sess_id;
              // echo " artist_id ".$artistid; //die;

              $evntstrtdatetime = $startdatetimeval;
...
//            echo "Creater id is ".$rqry_usr->creater_id;
            ...
            if($venuecreater!='' && $venuecreater > 0)
            {
                ...
                if($qryselectgrp)
                {
                    ...
                    }  //*********  end of ($grpid!='' && $grpid > 0)
                
                
                
            }  //********** end of ($venuecreater!='' && $venuecreater > 0)
...
//            die;
        ...
        //die;
            ...
            //******************* added for venue booking form split ends here
