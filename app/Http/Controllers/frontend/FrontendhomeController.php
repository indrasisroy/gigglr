...
class FrontendhomeController extends Controller
{
    ...
    public function index(Request $request)
    {
               ...
               //**** for message show purpose ends
                ...
                $user_master_db=$user_master_db->orderBy(DB::raw('RAND()'));
                $user_master_db=$user_master_db->take(1);
                $user_master_db=$user_master_db->get();
                
                //$user_master_data="SELECT bn.id,bn.status,bn.banner_image FROM banner as bn WHERE bn.status='1' ORDER BY RAND() LIMIT 1";
                //$user_master_db=DB::select($user_master_data);
                ...
                {
                         $banner_image=$user_master_db[0]->banner_image;
                ...
                $fieldnames=" welcome_text,default_radius,max_radius_limit,site_fax_no ";
                $wherear=array();
                $wherear['id']=1;
               ...
               $data['welcometxt_heading']=$welcometxt_heading;
              ...
              if(!empty($skl_master_db))
              {
               ...
               //*** fetch category (parent skill) related  data  ends 
                ...
                //****   random home search data fetch starts   ****//
                      ...
                      $home_search_db=$home_search_db->leftJoin('skill_master as smm', 'smm.id', '=', 'hs.skill_sub_id');
        
                      $home_search_db=$home_search_db->select(DB::raw('hs.id,hs.title,hs.location,hs.description,hs.skill_id,hs.skill_sub_id,hs.image_name,hs.image_title,sm.name,smm.name as genre_name'));
       ...
       public function registeruser(Request $request)
    {
            ...
            $gender=trim($request->input('gender','1'));

            /**############################  For G Plus Check Start  ###################################**/
            $is_g_plus_signup = addslashes(trim($request->input('is_g_plus_signup','')));
            /**############################  For G Plus Check Start  ###################################**/
           ...
           if(!empty($nickname))
           {
               $expldnameAR=explode(" ",$nickname);
               
               if(!empty($expldnameAR) && count($expldnameAR)==1)
               {
                   $first_name=$expldnameAR[0];
               }
               elseif(!empty($expldnameAR) && count($expldnameAR)==2)
               {
                   $first_name=$expldnameAR[0];
                   $last_name=$expldnameAR[1];
               }
                elseif(!empty($expldnameAR) && count($expldnameAR)==3)
               {
                    $first_name=$expldnameAR[0];                    
                    $middle_name=$expldnameAR[1];
                    $last_name=$expldnameAR[2];
               }
               else if(!empty($expldnameAR) && count($expldnameAR)>3)
               {
                    $first_name=$expldnameAR[0];                    
                    $middle_name=$expldnameAR[1];
                    ...
                    for($nn=2;$nn<count($expldnameAR);$nn++)
                    {
                        $last_name_data=$last_name_data.' '.$expldnameAR[$nn];
            ...
            $dataInsert['nickname']="Nickname";//$nickname;
           
            $dataInsert['first_name']=$first_name;
            $dataInsert['middle_name']=$middle_name;
            $dataInsert['last_name']=$last_name;
           ...
           if($chkvalid===true)
           {
                  ...
                  if(empty($id))
                  {
                        ...
                        //$updatear['seo_name']=$slugged_id;
                           $t=time();
                           $updatear['seo_name']=$t + $isInserted;
                 ...
                 // $activation_link=$bsurl."activateuser/".time()."/".base64_encode($isInserted);
                  /**############################  For G Plus Active Link Check Start  ###################################**/
                  if($is_g_plus_signup == 0)
                  {
                      $googletext = "Activate your account by clicking on the link below. ";
                      ...
                      /*##############################   Auto Active User Start Here   ##################################*/
                      $updateGplusVarify = array();
                      $updateGplusVarify['verify_status'] = 1;
                      $updateGplusVarify['status'] = 1;
                      $updateGplusVarifyResponse= DB::table('user_master')->where('id',$isInserted) ->update($updateGplusVarify);
                      /*##############################   Auto Active User Ends Here   ##################################*/

                  } 
                  //echo $is_g_plus_signup.'<br>'.$activation_link; die('Krishna Is dead');
                  /**############################  For G Plus Active Link Check Ends  ###################################**/
                  ...
                  $replacefrom =array('{GOOGLETEXT}','{NAME}','{USERNAME}','{PASSWORD}','{SITENAME}','{YEAR}','{BASE_URL}',"{LOGO_IMG}","{ACTIVATION_LINK}");
                  $replaceto =array($googletext,ucfirst($nickname),$email,$password,$sitename,$copyright_year,$bsurl,$logoIMG,$activation_link_text);
    ...
    public function checkregisterform($request,$id=0)
           {
                    ...
                    'nickname' => "required|alpha_spaces",
                    'email' =>array('required:required','regex:(^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,3})$)','unique:user_master'),
                    ...
                    'password_confirmation'=>"required",
                     ...
                     //  'nickname.unique'=>'* This Name is already in use',
                       "nickname.alpha_spaces"     => "* This Name may only contain letters and spaces.",
     ...
     public function loginuser(Request $request)
    {
           ...
           if($chkvalid===true)
           {
                ...
                if(!empty($user_master_db))
                {
                               ...
                               // $request->session()->flash('front_successmsgdata_sess', 'Hello, you are now logged in');
                ...
                $validator->after(function($validator)  use ($userData)  {
                       ...
                       $password=$userData['password'];
                        ...
                        //****fetch data to check starts
                                ...
                                if(!empty($user_master_db))
                                {
                                        ...
                                        if($verify_status==1)
                                        {
                                          if($user_type==3)
                                          {
                                             if($user_status==0)
                                             {
                                                      $validator->errors()->add('email', "This account has been deactivated by Admin.");
                                             }
                                ...
                                {
                                         $validator->errors()->add('email', "Whoops! that combination of email and password is incorrect");
                                }
           ...
           public function logout(Request $request)
                {
                    //*** set session  starts
                               ...
                               if ($request->session()->has('front_id_sess'))
                              {
                                      ...
                                      $this->delcook($sess_id,$r); // delete all cookies when user logs out 
                                ...
                                $prevurldata=url()->previous();
                              
                          //    $request->session()->flash('front_successmsgdata_sess', 'You have logged out');    
                               return redirect('/')->with(array('logoutstatus'=> 'yes','prevurldata'=>$prevurldata));
         ...
         public function forgotpassword(Request $request)
         {
                  ...
                  //*********get user details ends here
                   ...
                   //$logoIMG = asset('upload/email-template/source-file/footer-logo.png');
                    $logoIMG = asset('public/upload/settings-image/source-file/'.$Imgologo);
                  //*********Helper Function Starts here
                  $replacefrom =array('{Accountfname}','{USERNAME}','{PASSWORD}','{SITENAME}','{YEAR}','{BASE_URL}',"{LOGO_IMG}");
                       ...
                       $email=$userData['email'];
                        ...
                        //****fetch data to check starts
                                ...
                                if(!empty($user_master_db))
                                {
                                        ...
                                        if($user_status==0)
                                        {
                                                 $validator->errors()->add('email', "Unfortunately, this account is not active");
                                        }
                                        elseif($user_status==9)
                                        {
                                                 $validator->errors()->add('email', "This account has been deleted.");
                                ...
                                {
                                         $validator->errors()->add('email', "Sorry but that email is incorrect");
                                }
          ...
          public function fetchallcookies($request,$emailtocook,$passwordtocook)
          {
                  ...
                  //****** last added **************starts
                            ...
                            $rr = md5($front_id_sess);
                            Cookie::queue('cookuser_ID', $rr, 3600);
                            //*********inserting data in user master table for keep me signed in strats here
                            DB::table('user_master')
                            ->where('id', $front_id_sess)
                            ->update(['cookie_value' => $rr]);
                            //*********inserting data in user master table for keep me signed in ends here
          ...
          public function delcook($sess_id,$r)
          {
                  ...
                  $updatearr = array();
                  $updatearr['cookie_value']='';
                  DB::table('user_master')->where('id',$sess_id)->update($updatearr);
                  Cookie::queue(Cookie::forget('cookuser_ID'));
          }
            //****** delete cookies ends *****//
              ...
              //*** fetch data of user starts
               ...
               // $user_master_db=$user_master_db->where('um.status',1);
                ...
                else
                {
                           ...
                           $error_msg.="<p>Please fill up the form below.</p>";
                }
               ...
               $respAr['flag_id'] =$flag_id;
                ...
                $respAr['verify_status'] =$verify_status;
                $respAr['user_type'] =$user_type;
              ...
              //*** fetch data of user starts
               ...
               // $user_master_db=$user_master_db->where('um.status',1);
                ...
                if(!empty($user_master_db))
                {
                         ...
                         //*************checking for if already verified starts here 27-10-2016 
                         if($verify_status == 1)
                         {
                           // echo $verify_status;die;
                          $request->session()->flash('front_successmsgdata_sess','Your account is already activated.' );
                          return redirect('/');
                         }
                         //*************checking for if already verified ends here 27-10-2016 
                         ...
                         else
                         {
                          ...
                          $error_msg.="<p>Sorry but the activation link may have expired please contact support</p>";
                         }
                ...
                if(empty($error_msg) && ($user_status!=9) && ($verify_status!=1) )
                {
                           //*** update user_master table ends
                           $updatear=array();
                           $updatear["status"]=1;
          ...
          //***** for session check starts here added for if the session exists or not

          public function showsessionexiststrue(Request $request)
          {
             $r = $request->session()->get('front_id_sess');
             $t = $request->input('logIDchk');
             $respAr=array();
             $flag_id=0;
