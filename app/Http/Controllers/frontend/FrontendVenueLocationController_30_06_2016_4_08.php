...
// die;
 ...
 //****************insert into gig_notify table***********//
                   ...
                   $bookerqry = DB::table('user_master')
                    ...
                    $artistvenuegroup_qry = $this->booking_email($artistvenuegroupusrname,$artistvenuegroupemail,$bookerusrname,$bookingcategory_name,$bookinggenre_name,$address1val,$address2val,$towndata,$statename->state_name,$zipdata,$countryname->country_name,$bkngdate.' '.$strttim);
    ...
    //         {
                        ...
                        if($chkvalid===true)
                        {
                         ...
                         $closingtime = date("H:i:s", strtotime($closingtime));
                            ...
                            //*** update user_master table starts
                            $chkupd= DB::table('venue_master')->where('creater_id',$userid) ->update(
                           ['available_for' => $typeEvent ,'rate_amount' => $hourly_rate , 'security_figure' => $security_deposit , 'opening_time' => $openingtime , 'closing_time' => $closingtime , 'tech_spec' => $tech_spec ,'modified_date' => date('Y-m-d H:i:s')]
                            );
                            //*** update user_master table ends
                            if(!empty($chkupd))
                            {
