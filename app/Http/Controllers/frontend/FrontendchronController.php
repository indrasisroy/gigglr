...
class FrontendchronController extends Controller
{
     ...
     public function releaseescrowpayment(Request $request)
    {
         ...
         //3600 // 24
            ...
            $artist_qry=" SELECT
            ggm.`id`,ggm.`giguniqueid`,ggm.`total_amount`,ggm.`artist_id`,ggm.`type_flag`,ggm.`booker_id`,ggm.`booking_status`,ggm.`dispute_flag`,ggm.`payment_flag`,ggm.`event_type`,ggm.`booking_cancellation_fee`,ggm.`artist_security_deposit`,'".$php_srvr_datetime."' as just_now_datetime ,ggm.`event_end_date_time`,ggm.`event_end_date_time_ptz`, (time_to_sec(timediff('".$php_srvr_datetime."', `event_end_date_time_ptz`)) / 3600 ) as hours_event_passed
            ...
            ggm.`event_end_date_time_ptz` < '".$php_srvr_datetime."'
           ...
           //echo $artist_qry; //exit();
            ...
            if(!empty($releasegigrelateddataAr))
            {
                foreach($releasegigrelateddataAr as $releasegigrelateddataobj)
                {
                        ...
                        $total_amount_gm=$releasegigrelateddataobj->total_amount;
                        ...
                        //**** fetch data from gig_bidrequest table  starts *****
                            $gbr_data = DB::table('gig_bidrequest');
                            $gbr_data =$gbr_data->select('total_amount','id','gigmaster_id','artist_security_deposit');
                            $gbr_data=$gbr_data->where('gigmaster_id',$event_gig_id);                            
                            $gbr_data=$gbr_data->where('gig_bid_status','=','2');  
                            $gbr_data=$gbr_data->where('type_flag','=',$type_flag);
                            $gbr_data=$gbr_data->where('artist_id','=',$artist_id);
                            ...
                            if(!empty($gbr_data))
                            {
                                ...
                                $artist_security_deposit=$gbr_data->artist_security_deposit;
                            }
                    
                        //**** fetch data from gig_bidrequest table  ends *****
                ...
                //****** insert row in user_order table for atist to  make payment credited to artist wallet starts ******
                            ...
                            $site_com_credited_tosite=$site_com_debitedfrom_wallet; 
                ...
                //*** insert row in user_order table for (referrer)atist to  make  commision credited to his wallet starts ***
                            ...
                            $userdta_db=$userdta_db->select(DB::raw("id,user_id,emailid,referred_date,referral_expiry_date,DATEDIFF( '".$php_srvr_datetime."' , DATE_FORMAT( referred_date , '%Y-%m-%d %H:%i:%s' ) ) as refer_date_days_used,referrer_userid "));
                            $userdta_db=$userdta_db->where($wherefar);
                            ...
                            if(!empty($userdta_db))
                            {
                                ...
                                $refrlamttobededforsitecom=$ref_com_creditedtorerrer_wallet;
                                ...
                                $refereremail = $this->refereeremail($artistname,$email,'$'.$ref_com_creditedtorerrer_wallet,$giguniqueid);
                          ...
                          //****** insert row in user_order table for site commission credited for site starts ******                           
                            $isInserted=0;

                            $datedata=date('Y-m-d H:i:s');                            

                            $description="Site commission credited  for site ".$giguniqueid;
                            
                            
                            if(!empty($refrlamttobededforsitecom))
                            {
                                $site_com_credited_tosite=$site_com_credited_tosite-$refrlamttobededforsitecom;
                            }
                            
                                                        $dataorderInsert=array();
                                                        $dataorderInsert['payment_for']="SCC";
                                                       
                                                        
                                                        $dataorderInsert['payment_description']=$description;
                                                        $dataorderInsert['email']='';
                                                        $dataorderInsert['total_price']=$site_com_credited_tosite; 
                                                        $dataorderInsert['user_ip_address']=$logggedin_user_ip;
                                                        $dataorderInsert['debitorcredit']="C"; // C=> Credit , D=> Debit
                                                        $dataorderInsert['gigmaster_id']=$event_gig_id;
                                                        
                                                        $dataorderInsert['invoice_num']="INV-".time().rand(111111,999999);
                                                        $dataorderInsert['user_id']=0;
                                                        $dataorderInsert['create_date']=$datedata;
                                                        $dataorderInsert['modified_date']=$datedata;
                                                        $dataorderInsert['payment_status']='SUCCESS';
                        ...
                        //****** insert row in user_order table for site commission credited for site ends ******      
                               ...
                               $refrlamttobededforsitecom=$ref_com_creditedtorerrer_wallet;
                                ...
                                //****** update row in user_master table for atist to  make update wallet_amount data ends ******
                                    ...
                                    //************ Email send function for referer starts here
                                            ...
                                            $refereremail = $this->refereeremail($groupname,$email,'$'.$ref_com_creditedtorerrer_wallet,$giguniqueid);
                            ...
                            if(!empty($userdta_db))
                            {
                               ...
                               $refrlamttobededforsitecom=$ref_com_creditedtorerrer_wallet;
                                ...
                                //************ Email send function for referer starts here
                                            ...
                                            $refereremail = $this->refereeremail($venuename,$email,'$'.$ref_com_creditedtorerrer_wallet,$giguniqueid);
    ...
    public function cancelgigevent(Request $request)
      ...
      //***********Email function for refferer starts here
         ...
         public function refereeremail($artistname,$email,$ref_com_creditedtorerrer_wallet,$giguniqueid)
      {
            //echo "receiver_name=>".$receiver_name;die;
                   ...
                   // *********Helper Function Starts here
                    ...
                    $replacefrom =array('{ARTISTNAME}','{REFRERAMOUNT}','{SITENAME}','{YEAR}','{BASE_URL}','{LOGO_IMG}','{GIGUNIQUEID}');

                    $replaceto =array($artistname,$ref_com_creditedtorerrer_wallet,$sitename,$copyright_year,$bsurl,$logoIMG,$giguniqueid);
   ...
   public function cancelationpaymentreleased(Request $request)
    {
         ...
         $cancelgigeventmanualy = "SELECT gm . *,lc.currency_icon FROM `gig_master` AS gm LEFT OUTER JOIN `location_country` As lc ON lc.`id` = gm.`event_country`  WHERE gm.`booking_status` = '7' AND gm.`payment_flag` !='5' AND TIMESTAMPDIFF( HOUR , gm.`cancel_date` , '".$php_srvr_datetime."' ) > 24";
         ...
         if(!empty($cancelgigeventmanualyAr))
         {
                  ...
                  foreach($cancelgigeventmanualyAr as $data)
                  {
                           ...
                           $gig_cancel_amount = $data->booking_cancellation_fee;
                            ...
                            //**** fetch data from gig_bidrequest table  starts *****
                            $gbr_data = DB::table('gig_bidrequest');
                            $gbr_data =$gbr_data->select('total_amount','id','gigmaster_id','booking_cancellation_fee');
                            $gbr_data=$gbr_data->where('gigmaster_id',$gig_master_id);                            
                            $gbr_data=$gbr_data->where('gig_bid_status','=','2');
                            $gbr_data=$gbr_data->where('type_flag','=',$type_flag_data);
                            $gbr_data=$gbr_data->where('artist_id','=',$artist_id);
                            ...
                            if(!empty($gbr_data))
                            {
                                ...
                                $gig_total_amount=$gbr_data->total_amount;
                                $gig_cancel_amount=$gbr_data->booking_cancellation_fee;
                            }
                           ...
                           if($data->cancel_by_whom == '1'){
                                    ...
                                    if(!empty($check_booker_gig_sd_rlsd)){
                                             ...
                                             // $resttransferamoutartst = '';
                                              ...
                                              if($gig_cancel_amount == '0.00'){
                                                   $resttransferamout = $gig_total_amount; // to booker
                                                   $resttransferamoutartst = '';   
                                             }else{
                                                   $resttransferamout = $gig_total_amount -  $gig_cancel_amount; // to booker
                                                   $resttransferamoutartst = $gig_cancel_amount;   
                                             }
                                    }
                           }
    ...
    public function releaseofsecuritymoneytoagvwallet(Request $request)
    {
        ...
        //3600 //24
         ...
         $qryforsecuritymoney = "SELECT
            ggm.`id`,ggm.`giguniqueid`,ggm.`artist_id`,ggm.`type_flag`,ggm.`booking_status`,ggm.`dispute_flag`,ggm.`payment_flag`,ggm.`event_type`,ggm.`booking_cancellation_fee`,ggm.`artist_security_deposit`,'".$php_srvr_datetime."' as just_now_datetime ,ggm.`booking_accept_date`, (time_to_sec(timediff('".$php_srvr_datetime."', `booking_accept_date`)) / 3600 ) as hours_event_passed
            ...
            ggm.`booking_accept_date` < '".$php_srvr_datetime."'
...
public function eventnotifytouseremail(Request $request)
{
    //between 120 and 125
        ...
        $qryfornotifyuser = "SELECT
            ...
            ,'".$php_srvr_datetime."' as just_now_datetime ,ggm.`event_start_date_time`,ggm.`event_start_date_time_ptz`, (time_to_sec(timediff( `event_start_date_time_ptz`, '".$php_srvr_datetime."')) / 60 ) as minutes_event_remains
   ...
   // exit();
    //******** fetch pending bookings to check booking expiry to update to cancel  starts *********
        ...
        $php_srvr_datetime=date('Y-m-d H:i:s');
         ...
         $gig_pend_qry=" SELECT
            ggm.`id`,ggm.`giguniqueid`,ggm.`total_amount`,ggm.`artist_id`,ggm.`gigpostrequestflag`,ggm.`type_flag`,ggm.`booker_id`,ggm.`booking_status`,ggm.`dispute_flag`,ggm.`payment_flag`,ggm.`event_type`,ggm.`booking_cancellation_fee`,ggm.`artist_security_deposit`,'".$php_srvr_datetime."' as just_now_datetime ,ggm.`event_end_date_time`, (time_to_sec(timediff('".$php_srvr_datetime."', `event_end_date_time`)) / 60 ) as  min_event_passed,ggm.bk_req_date_tz,
            ggm.event_timezone,ggm.booking_req_date,ggm.bk_req_date_tz,ggm.request_expire,ggm.event_start_date_time
            ...
            if(!empty($gigpenddataAr))
            {
                foreach($gigpenddataAr as $gigpenddataArObj)
                {
                        ...
                        $event_start_date_time=$gigpenddataArObj->event_start_date_time;
                        $type_flag=$gigpenddataArObj->type_flag;

                        $giguniqueid=$gigpenddataArObj->giguniqueid;
                    
                    
                        $eventstartdatetime=date("Y-m-d H:i:s",strtotime($event_start_date_time));
                       ...
                       $gigpendbidreqAr=DB::select($gig_pend_bidreq_qry);
                        ...
                        if(!empty($gigpendbidreqAr))
                        {
                           //**** negotiation data exists
                            ...
                            foreach($gigpendbidreqAr as $gigpendbidrdata)
                            {
                                ...
                                // if expiration time is not  blank negotiation data exists
                                if($offer_expire_totaltime!='') 
                                {
                                ...
                                {
                                    // if expiration time is   blank negotiation data exists
                                   
                                    $curr_date_time=date("Y-m-d H:i:s");  
                                    
                                    // if current date time  is same or greater than event start time 
                                    if( strtotime($curr_date_time) >= strtotime($eventstartdatetime))
                                    {
                                         if($gigpostrequestflag==2) // if individual
                                                {
                                                    //***** update gig_bidrequest query starts *******
                        ...
                        else
                        {
                            ...
                            // if expiration time is not  blank negotiation data doesn't exists
                                 if($request_expire!='') 
                                {
                            ...
                            {
                                // if expiration time is   blank negotiation data doesn't exists
                                $curr_date_time=date("Y-m-d H:i:s");  
                                
                                /*
                                $tttt_chk=(strtotime($curr_date_time) >= strtotime($eventstartdatetime));
                                
                                echo '<br>---curr_date_time=>'.$curr_date_time.'---eventstartdatetime=>'.$eventstartdatetime;
                                echo "<br>";
                                var_dump($tttt_chk);
                                */
...
//**************   common email function if cancelled during negotiation process ends here ************
    ...
    /* notify booker to pay to wallet so that he can complete 
    booking process as Atist  already have accpeted 
    */
    
    public function canceleventonbookerinsufffund(Request $request)
    {
            ...
            */
            ...
            if(!empty($gigpenddataAr))
            {
                foreach($gigpenddataAr as $gigpenddataArObj)
                {
                        ...
                        $giguniqueid=$gigpenddataArObj->giguniqueid;
                        $booker_id=$gigpenddataArObj->booker_id;
                        $artist_id=$gigpenddataArObj->artist_id;
                    
                    
                        //*** fetch bookers wallet balance starts
                    
                        //*** fetch bookers wallet balance ends
                    
                        
                    
                    
                        if($gigpostrequestflag==1) // for gig post
                        {
                            /* fetch data related to this gig  where first 
                                acceptance was from Artist/Group/Venue 
                                side
                                */
                            
                                $userdta_dbp = DB::table("gig_bidrequest as ggbr");
                                $userdta_dbp=$userdta_dbp->select(DB::raw(" ggbr.id,ggbr.gigmaster_id,ggbr.giguniqueid,
                                ggbr.booker_id,ggbr.artist_id,ggbr.type_flag,ggbr.last_updated_by,
                                ggbr.first_accepted_by,ggbr.lub_type_flag,ggbr.fab_type_flag,
                                ggbr.booking_cancellation_fee,ggbr.bcf_lock_id,ggbr.bcf_lock_flag,
                                ggbr.artist_security_deposit,ggbr.asd_lock_id,ggbr.asd_lock_flag,ggbr.total_amount,
                                ggbr.ta_lock_id,ggbr.ta_lock_flag,ggbr.gig_description,ggbr.create_date,
                                ggbr.modified_date,ggbr.gig_bid_status,ggbr.offer_exp_datetime,                        ggbr.offer_expire_totaltime ,  (time_to_sec(timediff('".$php_srvr_datetime."', `modified_date`)) / 3600 ) as hours_event_passed  "));
                            
                                $userdta_dbp=$userdta_dbp->where('ggbr.gigmaster_id','=',$gigid);
                                $userdta_dbp=$userdta_dbp->where('ggbr.booker_id','=',$booker_id);
                                $userdta_dbp=$userdta_dbp->where('ggbr.gig_bid_status','=',1); 
                                $userdta_dbp=$userdta_dbp->where('ggbr.fab_type_flag','=','AGV');
                                //$userdta_dbp=$userdta_dbp->where('ggbr.first_accepted_by','=',$artist_id);
                                $userdta_dbp=$userdta_dbp->havingRaw(' (hours_event_passed > 24 ) ');
                            
                            
                            
                                $userdta_dbp=$userdta_dbp->orderBy('ggbr.id', 'desc');
                                $userdta_dbp=$userdta_dbp->get();
                            
                                /*
                                echo $gigid."--gig post gig_bidrequest***<pre>"; print_r($userdta_dbp);  echo "</pre>***";
                                */
                            
                                if(!empty($userdta_dbp))
                                {
                                   foreach($userdta_dbp as $userdta_db)
                                   {
                                       $artist_id=$userdta_db->artist_id;
                                       $gbr_id=$userdta_db->id;
                                       
                                       //*** fetch artist booking req price amount wallet balance starts
                    
                                       //*** fetch artist booking req price amount wallet balance ends
                                        ...
                                        $this->mailforcanceleventforinsuffnd($giguniqueid,$type_flag,$artist_id);
                        ...
                        elseif($gigpostrequestflag==2) // for individual request
                        {
                                /* fetch data related to this gig  where first 
                                acceptance was from Artist/Group/Venue 
                                side
                                */
                                ...
                                $userdta_db = DB::table("gig_bidrequest as ggbr");
                                $userdta_db=$userdta_db->select(DB::raw(" ggbr.id,ggbr.gigmaster_id,ggbr.giguniqueid,
                                ggbr.booker_id,ggbr.artist_id,ggbr.type_flag,ggbr.last_updated_by,
                                ggbr.first_accepted_by,ggbr.lub_type_flag,ggbr.fab_type_flag,
                                ggbr.booking_cancellation_fee,ggbr.bcf_lock_id,ggbr.bcf_lock_flag,
                                ggbr.artist_security_deposit,ggbr.asd_lock_id,ggbr.asd_lock_flag,ggbr.total_amount,
                                ...
                                $userdta_db=$userdta_db->where('ggbr.gigmaster_id','=',$gigid);
                                $userdta_db=$userdta_db->where('ggbr.booker_id','=',$booker_id);
                                $userdta_db=$userdta_db->where('ggbr.gig_bid_status','=',1); 
                                $userdta_db=$userdta_db->where('ggbr.fab_type_flag','=','AGV');
                                $userdta_db=$userdta_db->where('ggbr.first_accepted_by','=',$artist_id);
                                $userdta_db=$userdta_db->havingRaw(' (hours_event_passed > 24 ) ');
                            
                                $userdta_db=$userdta_db->orderBy('ggbr.id', 'desc');
                                $userdta_db=$userdta_db->first();
                            
                            
                                /* echo $gigid."--for individual request gig_bidrequest***<pre>"; print_r($userdta_db);  echo "</pre>***"; 
                                */
    ...
    function mailforcanceleventforinsuffnd($giguniqueid,$type_flag,$artist_id)
    {
        $nickname_bkr='';$emailto_bkr='';  $nickname_agv_data='';
        
        
        //****************  get event details starts here 

        $gigmstrqry = DB::table('gig_master')
        ->select('artist_id','type_flag','booker_id','gigpostrequestflag')
        ->where('giguniqueid',$giguniqueid)
        ->first();
        ...
        $bookrid = $gigmstrqry->booker_id;
        $artist_id = $gigmstrqry->artist_id;
        $gigpostrequestflag=$gigmstrqry->gigpostrequestflag;//1=>gig posted ,2=> individual post
        // echo "Here ".$typeval; //die;
        ...
        //************** To send mail to Artist/Group/Venue starts based on $gigpostrequestflag starts *********
        ...
        //************** To send mail to Artist/Group/Venue starts based on $gigpostrequestflag ends *********
                   ...
                   // echo "Here artist"; die;
                    // $selecttable = 'user_master';
                            ...
                            if(!empty($userdata))
                            {
                                ...
                                $emailto_agv = $userdata->email;
                                $nickname_agv = stripslashes($userdata->nickname);
                                $nickname_agv_data=$nickname_agv;
                            }
                            // echo " artist email is = > ".$emailto_agv;
                            // echo "<pre>";
                            // print_r($userdata);die;
        ...
        $nickname_agvown=" The Booker did not pay, this event will no longer valid";
                    
        $replaceto =array(ucfirst($nickname_agv),$sitename,$copyright_year,$bsurl,$logoIMG,$nickname_agvown,$giguniqueid);

        mailsnd($Temid=58,$replacefrom,$replaceto,$emailto_agv);
 //echo "Here artist"; die;
                        //******** mail function ends here 
                ...
                }else if($type_flag == '2')
                {
                        ...
                        $groupdata = DB::table('group_master')
                            ...
                            if(!empty($groupdata))
                            {
                               $emailto_agv = $groupdata->email;
                               $nickname_agv = stripslashes($groupdata->nickname);
                               // $nickname_agvown = '  Booker has not accepted the proposal of your  group '.stripslashes($groupdata->grpnkname)."  and it has passed 24 hours your aceptance.";
                                $nickname_agvown = ' The Booker did not pay, this event will no longer valid';
                                $nickname_agv_data=stripslashes($groupdata->grpnkname);
                            }
                ...
                }else if($type_flag == '3')
                {
                    // $selecttable = 'venue_master';
                            ...
                            if(!empty( $venuedata))
                            {
                               $emailto_agv = $venuedata->email;
                               $nickname_agv = stripslashes($venuedata->nickname);
                               // $nickname_agvown = ' Booker has not accepted the proposal of your venue '.stripslashes($venuedata->vnuenkname)."  and it has passed 24 hours your aceptance.";  
                               $nickname_agvown = ' The Booker did not pay, this event will no longer valid';
                                 
                                $nickname_agv_data=stripslashes($venuedata->vnuenkname);    
                            }
...
$replaceto =array(ucfirst(stripslashes($nickname_agv)),$sitename,$copyright_year,$bsurl,$logoIMG,$nickname_agvown,$giguniqueid);
        ...
        if($type_flag == '1')
        {
            $forartisttype="Artist : ".$nickname_agv_data;            
        }
        elseif($type_flag == '2')
        {
            $forartisttype="Group : ".$nickname_agv_data;            
        }
        elseif($type_flag == '3')
        {
            $forartisttype="Venue : ".$nickname_agv_data;            
        }
        ...
        $nickname_agvown=" You did not  accepted the proposal for ".$forartisttype." and it has passed 24 hours of their  aceptance. So the event has been cancelled.";

        $replacefrom =array('{User}','{SITENAME}','{YEAR}','{BASE_URL}','{LOGO_IMG}','{OTHERINFORMATION}','{EVENT-ID}');

        $replaceto =array(ucfirst($nickname_bkr),$sitename,$copyright_year,$bsurl,$logoIMG,$nickname_agvown,$giguniqueid);

        mailsnd($Temid=61,$replacefrom,$replaceto,$emailto_bkr);
        
        

        //******** mail function ends here 
        
        //************** To send mail to Booker ends ***********************************************************
