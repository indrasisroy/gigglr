...
class FrontendHelpController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        //$articlePage = DB::table('settings as ar');
        //$articlePage=$articlePage->select(DB::raw('ar.help_welcome_text'));   
        //$articlePagedata=$articlePage->first();
        ...
        $help_supportbypage=$help_supportbypage->orderby('supportbypage.order_id','ASC');
        ...
        $help_faqpage=$help_faqpage->orderby('fq.order_id','ASC');
...
//			
//			$usrmqry = DB::select("SELECT usrm.email AS uemail,usrm.first_name AS ufname,usrm.last_name AS ulname FROM user_master AS usrm WHERE usrm.id='".$frontloginuserid."'");
//			if(count($usrmqry)>0)
//			{
//				$uemail=$usrmqry[0]->uemail;
//				$ufname=$usrmqry[0]->ufname;
//				$ulname=$usrmqry[0]->ulname;
//				$usrmdata['usrmemail']=$uemail;
//				$usrmdata['usrmfname']=$ufname;
//				$usrmdata['usrmlname']=$ulname;
//			}
//			
...
//AND gm.id NOT IN (SELECT cancel_gig_id from contact_us)
//)
//	UNION
...
//		
//        //$articlePage = DB::table('settings as ar');
//        //$articlePage=$articlePage->select(DB::raw('ar.support_welcome_text'));   
//        //$articlePagedata=$articlePage->first();
//        //$datasupport['artile']=$articlePagedata;
//        //
//        //$datasupport['against_artist']=$against_artist;
//        $articlePage = DB::table('article as ar');
//        $articlePage=$articlePage->select(DB::raw('ar.description,ar.title'));
//        $articlePage=$articlePage->where('ar.status', 1);
//        $articlePage=$articlePage->where('ar.id', 14);        
//        $articlePagedata=$articlePage->first();
//		
//		$datasupport['artile']=$articlePagedata;
...
//    }
	...
	public function support(Request $request)
    {
		...
		if(isset($sess_id) && $sess_id!=0)
		{
			...
			$cancelbookingqry = DB::select(
"(SELECT gm.id AS gigmasterid, gm.giguniqueid AS uniquegigid, gm.type_flag AS flagtype, gm.artist_id AS artistid, gm.booker_id AS other_id, um.nickname AS artistname, um.email AS artistmail, IF('1'='1', 'artist', 'booker') AS bookerorartist
FROM gig_master AS gm
LEFT JOIN user_master AS um ON um.id=gm.artist_id
WHERE gm.booker_id='".$frontloginuserid."'
AND gm.artist_id!='0'
AND gm.type_flag='1'
AND gm.booking_status='1'
AND (gm.event_start_date_time_ptz > '".$presentdt."')
AND gm.id NOT IN (SELECT gig_id from resolve_dispute)
AND gm.id NOT IN (SELECT cancel_gig_id from contact_us)
)
	UNION
(SELECT gm.id AS gigmasterid, gm.giguniqueid AS uniquegigid, gm.type_flag AS flagtype, gm.artist_id AS artistid, gm.booker_id AS other_id, grp.nickname AS artistname, um.email as artistmail, IF('1'='1', 'group', 'booker') AS bookerorartist
FROM gig_master AS gm
LEFT JOIN group_master AS grp ON grp.id=gm.artist_id
LEFT JOIN user_master AS um ON um.id=grp.creater_id
WHERE gm.booker_id='".$frontloginuserid."'
...
AND gm.id NOT IN (SELECT cancel_gig_id from contact_us)
	...
	UNION
(SELECT gm.id AS gigmasterid, gm.giguniqueid AS uniquegigid, gm.type_flag AS flagtype, gm.artist_id AS artistid, gm.booker_id AS other_id, ven.nickname AS artistname, um.email AS artistmail, IF('1'='1', 'venue', 'booker') AS bookerorartist
FROM gig_master AS gm
LEFT JOIN venue_master AS ven ON ven.id=gm.artist_id
LEFT JOIN user_master AS um ON um.id=ven.creater_id
WHERE gm.booker_id='".$frontloginuserid."'
...
AND gm.id NOT IN (SELECT cancel_gig_id from contact_us)
    ...
    UNION
(SELECT gm.id AS gigmasterid, gm.giguniqueid AS uniquegigid, gm.type_flag AS flagtype, gm.booker_id AS artistid, gm.artist_id AS other_id, um.nickname AS artistname, um.email AS artistmail, IF('1'='1', 'booker', 'artist') AS bookerorartist
FROM gig_master AS gm
LEFT JOIN user_master as um ON um.id=gm.booker_id
WHERE ( (gm.artist_id='".$frontloginuserid."' AND gm.type_flag='1')
OR (gm.artist_id IN (SELECT grm.id FROM group_master AS grm WHERE grm.creater_id='".$frontloginuserid."' AND grm.status='1') AND gm.type_flag='2')
OR (gm.artist_id IN (SELECT venu.id FROM venue_master AS venu WHERE venu.creater_id='".$frontloginuserid."' AND venu.status='1') AND gm.type_flag='3') )
AND gm.booking_status='1'
AND (gm.event_start_date_time_ptz > '".$presentdt."')
AND gm.id NOT IN (SELECT gig_id from resolve_dispute)
AND gm.id NOT IN (SELECT cancel_gig_id from contact_us)
			...
			if(count($cancelbookingqry)>0)
			{
				foreach( $cancelbookingqry as $cancelbookingdata )
				{
					...
					$cancelgigartistmail=$cancelbookingdata->artistmail;
					$cancelgiguniqueid=$cancelbookingdata->uniquegigid;
					$cancelgigartistnameb4=$cancelbookingdata->artistname;
					$cancelgigartistname=stripslashes($cancelgigartistnameb4);
					$cancelgigartisttype=$cancelbookingdata->bookerorartist;
					$cancelgigartisttypeflag=$cancelbookingdata->flagtype;
					$cancelgigartistidto=$cancelbookingdata->artistid;
					$cancelgigotheridfrom=$cancelbookingdata->other_id;
					if($cancelgigartisttype=='booker')
					{
						$cancelgigbywhom='artist';
					}
					else{
						$cancelgigbywhom='booker';
					}
					$cancelbooking_content['element']=$cancelgigmastermainid."-".$cancelgigartistmail."-".$cancelgigartistidto."-".$cancelgigartisttypeflag."-".$cancelgigotheridfrom."-".$cancelgigbywhom;
					$cancelbooking_content['content']=$cancelgigartistname." (".$cancelgigartisttype.") - ".$cancelgiguniqueid." (unique gig ID)";
				...
				$cancelbooking_content['element']='';
				$cancelbooking_content['content']='No event is available';
				array_push($cancel_bookinglist,$cancelbooking_content);
			}
		}
		else
        ...
        $against_artist_nest_query=''; $against_artist_query_arr=''; $artquarr=array(); $against_artist_query=array(); $gid=''; $endevent='';
		$against_group_nest_query=''; $against_group_query_arr=''; $grpquarr=array(); $against_group_query=array(); $gid=''; $endevent='';
		$against_venue_nest_query=''; $against_venue_query_arr=''; $venquarr=array(); $against_venue_query=array(); $gid=''; $endevent='';
		$against_booker_nest_query=''; $against_booker_query_arr=''; $bkrquarr=array(); $against_booker_query=array(); $gid=''; $endevent='';
		$reasonarrqry='';
        $datasupport=array(); $against_artist=array(); $against_group=array(); $against_venue=array(); $against_booker=array(); $against_artist_content=array(); $against_group_content=array(); $against_venue_content=array(); $against_booker_content=array(); $reasonarrcontent=array(); $reasonarr=array();
        ...
        if(isset($sess_user_id) && $sess_user_id!=0)
        {
			$against_artist_nest_query = DB::select("SELECT id,(event_end_date_time_ptz + INTERVAL 24 HOUR) AS event_end FROM gig_master WHERE booker_id='".$sess_user_id."' AND type_flag='1' AND booking_status='1' AND artist_id!='0'");
			//print_r($against_artist_nest_query);die;
			
			foreach($against_artist_nest_query as $artnest)
			{
				$gid=$artnest->id;
				$endevent=$artnest->event_end;
				
				$against_artist_query_arr = DB::select(
"SELECT gm.id AS gigmasterid, gm.giguniqueid AS uniquegigid, gm.artist_id AS artistid, um.nickname AS artistname, um.email AS artistmail
FROM gig_master AS gm
LEFT JOIN user_master AS um ON um.id=gm.artist_id
WHERE gm.id='".$gid."'
AND ('".$endevent."' > '".$presentdt."')
AND gm.id NOT IN (SELECT gig_id from resolve_dispute)"
				);
				
				if(count($against_artist_query_arr)>0)
				{
					$artquarr['gigmasterid']=$against_artist_query_arr[0]->gigmasterid;
					$artquarr['uniquegigid']=$against_artist_query_arr[0]->uniquegigid;
					$artquarr['ida']=$against_artist_query_arr[0]->artistid;
					$artquarr['artistname']=$against_artist_query_arr[0]->artistname;
					$artquarr['artistmail']=$against_artist_query_arr[0]->artistmail;
					if(count($artquarr)>0)
					{
						array_push($against_artist_query,$artquarr);
					}
				}
			}
...
//AND gm.id NOT IN (SELECT gig_id from resolve_dispute)"
			...
			$against_group_nest_query = DB::select("SELECT id,(event_end_date_time_ptz + INTERVAL 24 HOUR) AS event_end FROM gig_master WHERE booker_id='".$sess_user_id."' AND type_flag='2' AND booking_status='1' AND artist_id!='0'");
			//print_r($against_artist_nest_query);die;
			
			foreach($against_group_nest_query as $grpnest)
			{
				$gid=$grpnest->id;
				$endevent=$grpnest->event_end;
				
				$against_group_query_arr = DB::select(
"SELECT gm.id AS gigmasterid, gm.giguniqueid AS uniquegigid, gm.artist_id AS groupid, grp.nickname AS groupname, grp.creater_id AS groupcraetorid, um.nickname AS groupcreatorname, um.email as groupcreatormail
FROM gig_master AS gm
LEFT JOIN group_master AS grp ON grp.id=gm.artist_id
LEFT JOIN user_master AS um ON um.id=grp.creater_id
...
AND gm.id NOT IN (SELECT gig_id from resolve_dispute)"
				...
				if(count($against_group_query_arr)>0)
				{
					$grpquarr['gigmasterid']=$against_group_query_arr[0]->gigmasterid;
					$grpquarr['uniquegigid']=$against_group_query_arr[0]->uniquegigid;
					$grpquarr['ida']=$against_group_query_arr[0]->groupid;
					$grpquarr['groupname']=$against_group_query_arr[0]->groupname;
					$grpquarr['groupcreatormail']=$against_group_query_arr[0]->groupcreatormail;
					if(count($grpquarr)>0)
					{
						array_push($against_group_query,$grpquarr);
					}
				}
			}
...
//			);
			...
			$against_venue_nest_query = DB::select("SELECT id,(event_end_date_time_ptz + INTERVAL 24 HOUR) AS event_end FROM gig_master WHERE booker_id='".$sess_user_id."' AND type_flag='3' AND booking_status='1' AND artist_id!='0'");
			//print_r($against_artist_nest_query);die;
			
			foreach($against_venue_nest_query as $vennest)
			{
				$gid=$vennest->id;
				$endevent=$vennest->event_end;
				
				$against_venue_query_arr = DB::select(
"SELECT gm.id AS gigmasterid, gm.giguniqueid AS uniquegigid, gm.artist_id AS venueid, ven.nickname AS venuename, ven.creater_id AS venuecreatorid, um.nickname AS venuecreatorname, um.email AS venuecreatormail
FROM gig_master AS gm
LEFT JOIN venue_master AS ven ON ven.id=gm.artist_id
LEFT JOIN user_master AS um ON um.id=ven.creater_id
...
AND gm.id NOT IN (SELECT gig_id from resolve_dispute)"
				...
				if(count($against_venue_query_arr)>0)
				{
					$venquarr['gigmasterid']=$against_venue_query_arr[0]->gigmasterid;
					$venquarr['uniquegigid']=$against_venue_query_arr[0]->uniquegigid;
					$venquarr['ida']=$against_venue_query_arr[0]->venueid;
					$venquarr['venuename']=$against_venue_query_arr[0]->venuename;
					$venquarr['venuecreatormail']=$against_venue_query_arr[0]->venuecreatormail;
					if(count($venquarr)>0)
					{
						array_push($against_venue_query,$venquarr);
					}
				}
			}
...
//			);
			...
			$against_booker_nest_query = DB::select("SELECT id,(event_end_date_time_ptz + INTERVAL 24 HOUR) AS event_end FROM gig_master WHERE ( (artist_id='".$sess_user_id."' AND type_flag='1')
OR (artist_id IN (SELECT grm.id FROM group_master AS grm WHERE grm.creater_id='".$sess_user_id."' AND grm.status='1') AND type_flag='2')
OR (artist_id IN (SELECT venu.id FROM venue_master AS venu WHERE venu.creater_id='".$sess_user_id."' AND venu.status='1') AND type_flag='3') )
AND booking_status='1'");
			//print_r($against_booker_nest_query);die;
			
			foreach($against_booker_nest_query as $bkrnest)
			{
				$gid=$bkrnest->id;
				$endevent=$bkrnest->event_end;
				
				$against_booker_query_arr = DB::select(
"SELECT gm.id AS gigmasterid, gm.giguniqueid AS uniquegigid, gm.booker_id AS bookerid, um.nickname AS bookername, um.email AS bookermail
FROM gig_master AS gm
LEFT JOIN user_master as um ON um.id=gm.booker_id
WHERE gm.id='".$gid."'
...
AND gm.id NOT IN (SELECT gig_id from resolve_dispute)"
				...
				if(count($against_booker_query_arr)>0)
				{
					$bkrquarr['gigmasterid']=$against_booker_query_arr[0]->gigmasterid;
					$bkrquarr['uniquegigid']=$against_booker_query_arr[0]->uniquegigid;
					$bkrquarr['ida']=$against_booker_query_arr[0]->bookerid;
					$bkrquarr['bookername']=$against_booker_query_arr[0]->bookername;
					$bkrquarr['bookermail']=$against_booker_query_arr[0]->bookermail;
					if(count($bkrquarr)>0)
					{
						array_push($against_booker_query,$bkrquarr);
					}
				}
			}
...
//			);
       ...
       //print_r($against_artist_query);die;
			...
			if(count($against_artist_query)>0)
			{
				foreach( $against_artist_query as $against_artist_data )
				{
					...
					$gigartistmail=$against_artist_data['artistmail'];
					$giguniqueid=$against_artist_data['uniquegigid'];
					$artistnameb4=$against_artist_data['artistname'];
					$artistname=stripslashes($artistnameb4);
					$against_artist_content['element']=$gigmastermainid."-".$gigartistmail;
					$against_artist_content['content']=$artistname." - ".$giguniqueid." (unique gig ID)";
				...
				$against_artist_content['element']='';
				$against_artist_content['content']='No event is available';
			...
			if(count($against_group_query)>0)
			{
				foreach( $against_group_query as $against_group_data )
				{
					...
					$groupcreatormail=$against_group_data['groupcreatormail'];
					$giguniqueid=$against_group_data['uniquegigid'];
					$groupnameb4=$against_group_data['groupname'];
					$groupname=stripslashes($groupnameb4);
					$against_group_content['element']=$gigmastermainid."-".$groupcreatormail;
					$against_group_content['content']=$groupname." - ".$giguniqueid." (unique gig ID)";
				...
				$against_group_content['element']='';
				$against_group_content['content']='No event is available';
			...
			if(count($against_venue_query)>0)
			{
				foreach( $against_venue_query as $against_venue_data )
				{
					...
					$venuecreatormail=$against_venue_data['venuecreatormail'];
					$giguniqueid=$against_venue_data['uniquegigid'];
					$venuenameb4=$against_venue_data['venuename'];
					$venuename=stripslashes($venuenameb4);
					$against_venue_content['element']=$gigmastermainid."-".$venuecreatormail;
					$against_venue_content['content']=$venuename." - ".$giguniqueid." (unique gig ID)";
				...
				$against_venue_content['element']='';
				$against_venue_content['content']='No event is available';
			...
			if(count($against_booker_query)>0)
			{
				foreach( $against_booker_query as $against_booker_data )
				{
					...
					$bookermail=$against_booker_data['bookermail'];
					$giguniqueid=$against_booker_data['uniquegigid'];
					$bookernameb4=$against_booker_data['bookername'];
					$bookername=stripslashes($bookernameb4);
					$against_booker_content['element']=$gigmastermainid."-".$bookermail;
					$against_booker_content['content']=$bookername." - ".$giguniqueid." (unique gig ID)";
				...
				$against_booker_content['element']='';
				$against_booker_content['content']='No event is available';
	...
	public function contactsupportsubfunc(Request $request)
	{
		...
		//{
			...
			if(isset($gigselectdata) && $gigselectdata!='' && $gigselectdata>0){
				...
				$cancelartisttoid=$split_cancelgig_array[2];
				$cancelgigflagtypearr=$split_cancelgig_array[3];
				$cancelartistfromid=$split_cancelgig_array[4];
				$cancelartistfromtypearr=$split_cancelgig_array[5];
				
				if($cancelartistfromtypearr=='booker')
				{
					$cancelgigflagtype=1;
					
					$cancelartistfromtype=2;
				}
				else{
					$cancelgigflagtype=$cancelgigflagtypearr;
					
					$cancelartistfromtype=1;
				}
			...
			if($chkvalid===true)
			{
				...
				if($contactreason==6)
				{
					...
					$isUpdated1 = DB::table('gig_master')
								->where('id', $gigselect)
								->update(['booking_status' => '7','cancel_manual_id'=>$cancelartistfromid,'cancel_by_whom'=>$cancelartistfromtype,'cancel_type_flag'=>$cancelgigflagtype,'cancel_date'=>$conDate]);
				}
				else{
				...
				if($contactreason==6)
				{
					...
					$gig_master_db_sub11=$gig_master_db_sub11->select(DB::raw('giguniqueid,event_city,type_flag,event_start_date_time'));
					...
					$canceluniqueidforgig=$gig_master_db_sub11->giguniqueid;
					
					$replacefrom =array('{LOGO_IMG}','{BASE_URL}','{TOUSER}','{FROMUSER}','{UNIQUEIDGIG}','{GIGDATE}','{GIGTOWN}','{SITENAME}','{YEAR}');
					$replaceto =array($logoIMG,$bsurl,$toname,$fromname,$canceluniqueidforgig,$cancelstart,$cancel_event_city,$sitename,$copyright_year);
					
					mailsnd($Temid=46,$replacefrom,$replaceto,$cancelartistemail);
					
					
					$replacefromme =array('{LOGO_IMG}','{BASE_URL}','{FROMUSER}','{UNIQUEIDGIG}','{GIGDATE}','{GIGTOWN}','{SITENAME}','{YEAR}');
					$replacetome =array($logoIMG,$bsurl,$fromname,$canceluniqueidforgig,$cancelstart,$cancel_event_city,$sitename,$copyright_year);
					
					mailsnd($Temid=47,$replacefromme,$replacetome,$conemail);
				}
			}
			else{
				$flag_id = 0;
	...
	public function checkcontactsupportform($request,$sess_id=0)
    {
        $validator = Validator::make($request->all(),
			...
			[
				'contactreason.required'=>'* You forgot to select a reason for contact',
				'confname.required'=>'* Please provide your first name',
				'conemail.required'=>'* Please provide your email address',
				'conemail.email'=>'* Email should be of valid format',
				'condesc.required'=>'* Please place your concern or query in the message area',
			]
		);
	...
	public function refreshingpicker(Request $request)
	{	
		$respdataAr=array();
		$frontloginuserid='';
		...
		if(isset($sess_id) && $sess_id>0)
		{
			...
			$againstbookerdatatonxt=array(); $againstvenuedatatonxt=array(); $againstgroupdatatonxt=array(); $againstartistdatatonxt=array(); $canceldatatonxt=array();
	...
	AND gm.id NOT IN (SELECT cancel_gig_id from contact_us)
			...
			$against_artist_nest_query = DB::select("SELECT id,(event_end_date_time_ptz + INTERVAL 24 HOUR) AS event_end FROM gig_master WHERE booker_id='".$frontloginuserid."' AND type_flag='1' AND booking_status='1' AND artist_id!='0'");
...
AND gm.id NOT IN (SELECT gig_id from resolve_dispute)"
			...
			$against_group_nest_query = DB::select("SELECT id,(event_end_date_time_ptz + INTERVAL 24 HOUR) AS event_end FROM gig_master WHERE booker_id='".$frontloginuserid."' AND type_flag='2' AND booking_status='1' AND artist_id!='0'");
...
AND gm.id NOT IN (SELECT gig_id from resolve_dispute)"
			...
			$against_venue_nest_query = DB::select("SELECT id,(event_end_date_time_ptz + INTERVAL 24 HOUR) AS event_end FROM gig_master WHERE booker_id='".$frontloginuserid."' AND type_flag='3' AND booking_status='1' AND artist_id!='0'");
...
AND gm.id NOT IN (SELECT gig_id from resolve_dispute)"
			...
			$against_booker_nest_query = DB::select("SELECT id,(event_end_date_time_ptz + INTERVAL 24 HOUR) AS event_end FROM gig_master WHERE ( (artist_id='".$frontloginuserid."' AND type_flag='1')
OR (artist_id IN (SELECT grm.id FROM group_master AS grm WHERE grm.creater_id='".$frontloginuserid."' AND grm.status='1') AND type_flag='2')
OR (artist_id IN (SELECT venu.id FROM venue_master AS venu WHERE venu.creater_id='".$frontloginuserid."' AND venu.status='1') AND type_flag='3') )
AND booking_status='1'");
...
AND gm.id NOT IN (SELECT gig_id from resolve_dispute)"
			...
			$canceldatatonxt['cancel_bookinglist']=$cancel_bookinglist;
			$againstartistdatatonxt['against_artist']=$against_artist;
			$againstgroupdatatonxt['against_group']=$against_group;
			$againstvenuedatatonxt['against_venue']=$against_venue;
			$againstbookerdatatonxt['against_booker']=$against_booker;
			
			$view_obj_cancel_gig = View::make('front.help.cancelgignextview',$canceldatatonxt);
			$contents_cancelgig = $view_obj_cancel_gig->render();
			$view_obj_against_artist = View::make('front.help.againstartistnextview',$againstartistdatatonxt);
			$contents_againstartist = $view_obj_against_artist->render();
			$view_obj_against_group = View::make('front.help.againstgroupnextview',$againstgroupdatatonxt);
			$contents_againstgroup = $view_obj_against_group->render();
			$view_obj_against_venue = View::make('front.help.againstvenuenextview',$againstvenuedatatonxt);
			$contents_againstvenue = $view_obj_against_venue->render();
			$view_obj_against_booker = View::make('front.help.againstbookernextview',$againstbookerdatatonxt);
			$contents_againstbooker = $view_obj_against_booker->render();
				
			$respdataAr['contents_cancelgig']=$contents_cancelgig;
			$respdataAr['contents_againstartist']=$contents_againstartist;
			$respdataAr['contents_againstgroup']=$contents_againstgroup;
			$respdataAr['contents_againstvenue']=$contents_againstvenue;
			$respdataAr['contents_againstbooker']=$contents_againstbooker;
		}
		else{
			$respdataAr['contents_cancelgig']=array();
			$respdataAr['contents_againstartist']=array();
			$respdataAr['contents_againstgroup']=array();
			$respdataAr['contents_againstvenue']=array();
			$respdataAr['contents_againstbooker']=array();
		}
