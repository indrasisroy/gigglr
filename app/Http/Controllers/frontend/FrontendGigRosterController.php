...
//          print_r($qryforcountfirstaccept);
    ...
    {  
     //echo "Hello booking by booker and artist ";
      ...
      //****************  amount in wallet present ***************

      $rerurnArray['amount_have'] = 999990975; 
      ...
      // //************   get complete details from gigbid request table starts here 

      // $getcompletedetailsdata_gigbid = $this->customfuncgigbiddetsils($gigunique_id);
      // // echo "<pre>";
      // // print_r($getcompletedetailsdata_gigbid);die;

      // $getingbookerid_gigbid = $getcompletedetailsdata_gigbid->booker_id;
      // $getingartistid_gigbid = $getcompletedetailsdata_gigbid->artist_id;
      // $getingtypeflag_gigbid = $getcompletedetailsdata_gigbid->type_flag;
              ...
              if($countvaldata > 0)
              {
                ...
                if($check_alreadyexceptedval!='' && $check_alreadyexceptedval>0)
                {
                 ...
                 //*********  Here we are going to complete the accept process from both end by release escrow amount
                  ...
                  $escrowpaymentdone = $this->escrowonacceptingbkrequest($booker_id,$bkridtype=1,$gigmaster_id,$getingartistid_gigbid,$getingtypeflag_gigbid);
                  ...
                  //***********  if escrow amount is released then update the table starts here
                    ...
                    $updatedatatogigbidreqarray['offer_expire_totaltime'] = '';
                    ...
                    if($secondacptqry)
                    {
                       ...
                       if($booker_id == $sessionid)
                       {
                         $rerurnArray['amount_have'] = (float) $escrowpaymentdone['wallet_update_amt']; 
                       }
                       


                       $rerurnArray['flag'] = 1; 
                       $rerurnArray['msg'] = "Congratulations the booking has been accepted";
                        echo json_encode($rerurnArray);
                        exit();
                    }
...
//**************add in 16-12-16 start ************//
      ...
      if(!empty($gig_master_details)){
       ...
       if(!empty($artist_mail_details)){
        ...
        $replaceto =array(ucfirst(stripslashes($artist_name))," by ".$booker_name." as a Booker",$gig_master_details->giguniqueid,$sitename,$copyright_year,$bsurl,$logoIMG," cancelled ");
        mailsnd($Temid=50,$replacefrom,$replaceto,$artist_email);
        //************* Send mail to artist ***********//
        
        //************* Send mail to booker ***********//
        $replacefrom =array('{USER}','{BY_WHOM}','{EVENT_NAME}','{SITENAME}','{YEAR}','{BASE_URL}',"{LOGO_IMG}","{CNCLDCL}");
        $replaceto =array(ucfirst(stripslashes($booker_name))," by you ",$gig_master_details->giguniqueid,$sitename,$copyright_year,$bsurl,$logoIMG," cancelled ");
...
//                'asd_lock_flag'=>
      ...
      if(!empty($gig_master_details)){
       ...
       if(!empty($artist_mail_details)){
        ...
        //************* Send mail to booker***********//
        $replacefrom =array('{USER}','{BY_WHOM}','{EVENT_NAME}','{SITENAME}','{YEAR}','{BASE_URL}',"{LOGO_IMG}","{CNCLDCL}");
        $replaceto =array(ucfirst(stripslashes($booker_name))," by ".$artist_name." as a Artist",$gig_master_details->giguniqueid,$sitename,$copyright_year,$bsurl,$logoIMG," declined ");
        mailsnd($Temid=50,$replacefrom,$replaceto,$booker_email);
        //************* Send mail to booker ***********//
        
        //************* Send mail to artist ***********//
        $replacefrom =array('{USER}','{BY_WHOM}','{EVENT_NAME}','{SITENAME}','{YEAR}','{BASE_URL}',"{LOGO_IMG}","{CNCLDCL}");
        $replaceto =array(ucfirst(stripslashes($artist_name))," by you ",$gig_master_details->giguniqueid,$sitename,$copyright_year,$bsurl,$logoIMG," declined ");
        mailsnd($Temid=50,$replacefrom,$replaceto,$artist_email);
        //************* Send mail to artist ***********//
    ...
    function cheking_distance($lat1,$lat2,$long1,$long2){
     ...
     $goople_distance_api = "https://maps.googleapis.com/maps/api/distancematrix/json?origins=".$fAddress."&destinations=".$sAddress."&mode=driving&language=en-EN&key=".$settingsgoogleapi;
    ...
    //***********************  custom function to get event details ends here
     ...
     //*********************** custom function to get event details  starts here  modified starts for booking form split
    public function customfuncgigbiddetsils($eventid)
    {
     $eventdetsilsqry = DB::table('gig_bidrequest')->where('giguniqueid',$eventid)->first();   
        return $eventdetsilsqry;
    }
    //***********************  custom function to get event details ends here
...
// response => Array ( [converteddatetime] => 2017-01-17 02:00:00 [reqdatetime] => 2017-01-16 15:00:00 [fromtimezone] => UTC [totimezone] => Australia/Sydney )
      ...
      $bookerdataemail='';
          ...
          if($fetchingdataqrybkr)
          {
              ...
              $bookerdataemail = $fetchingdataqrybkr->email;
                ...
                $replaceto =array(ucfirst(stripslashes($agvnickname)),ucfirst(stripslashes($bookerdatanickname)),$sitename,$copyright_year,$bsurl,$logoIMG,$gigunique_id);
                    mailsnd($Temid=56,$replacefrom,$replaceto,$agvemail);
                  }else
                  {
                    
                    $replacefrom =array("{RESPOND_TO}","{RESPOND_BY}",'{SITENAME}','{YEAR}','{BASE_URL}',"{LOGO_IMG}","{EVENTID}");
                $replaceto =array(ucfirst(stripslashes($bookerdatanickname)),ucfirst(stripslashes($agvnickname)),$sitename,$copyright_year,$bsurl,$logoIMG,$gigunique_id);
    ...
    //  echo "firstacceptornotflag == >".$firstacceptornotflag;
...
$logoIMG = asset('public/upload/settings-image/source-file/'.$Imgologo);
    ...
    //**********************  separete escrow function starts here  ***********************
     ...
     function escrowonacceptingbkrequest($bkrid=0,$bkridtype=1,$bkgigid=0,$art_id,$art_type)
    {
            ...
            if(!empty($bkrid) && !empty($bkridtype) && !empty($bkgigid))
            {
                ...
                if(!empty($gig_master_details))
                {
                    ...
                    //****************  fetching gig bid request data starts here****************
                      ...
                      $gbr_data=$gbr_data->where('artist_id','=',$art_id); 
                      $gbr_data=$gbr_data->where('type_flag','=',$art_type); 
                    ...
                    // echo "Total amount ===> ".$total_amount;die;
               ...
               // echo $total_amount."---".$wallet_amount;
                if($payment_flag==0)
                {
                   // echo "I am Here ".$payment_description."tt  => ".$total_amount."bkid=>".$booker_id;
                    if(!empty($booker_id))
                    {
                     // echo "I am Here ".$payment_description;die;
                      // $total_amount>0 && ($total_amount < $wallet_amount) && !empty($booker_id)
        ...
        //echo "<pre>";print_r($escrowResp);die;
