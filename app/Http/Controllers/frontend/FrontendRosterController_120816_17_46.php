...
( SELECT IF( ( SELECT COUNT(gb.id) AS gbid FROM gig_bidrequest AS gb WHERE gb.gigmaster_id=gm.id AND gb.artist_id=grp.id )>0, '1', '0' ) AS ggbid, IF( '1' = '1', 'artist', 'booker' ) AS bookerorartist, gm.*, gm.event_start_date_time AS start, gsr.category, gsr.genre, IF( sm1.name IS NULL, 'N/A', sm1.name ) AS category_name, IF( sm2.name IS NULL, 'N/A', sm2.name ) AS genre_name, IF( um1.nickname IS NULL, 'N/A', um1.nickname ) AS booker_name, grp.nickname AS assumed_artistname
...
( SELECT IF( ( SELECT COUNT(gb.id) AS gbid FROM gig_bidrequest AS gb WHERE gb.gigmaster_id=gm.id AND gb.artist_id='".$user_id."' )>0, '1', '0' ) AS ggbid, IF( '1' = '1', 'artist', 'booker' ) AS bookerorartist, gm.*, gm.event_start_date_time AS start, gsr.category, gsr.genre, IF( sm1.name IS NULL, 'N/A', sm1.name ) AS category_name, IF( sm2.name IS NULL, 'N/A', sm2.name ) AS genre_name, IF( um1.nickname IS NULL, 'N/A', um1.nickname ) AS booker_name, usms.nickname AS assumed_artistname 
FROM gig_master AS gm 
JOIN gig_skill_rel AS gsr ON gsr.gigmaster_id = gm.id
INNER JOIN skill_master AS sm1 ON gsr.category = sm1.id 
LEFT JOIN skill_master AS sm2 ON gsr.genre = sm2.id 
