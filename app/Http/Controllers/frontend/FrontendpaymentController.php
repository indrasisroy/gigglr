...
public function paytowallet(Request $request)
     ...
     public function paymentwalletprocess(Request $request)
    {
            ...
            //$price = trim($request->input('price'));
                ...
                $amountdata=round(floatval($amountdata),2);
                 ...
                 if (fmod($amountdata, 1) == 0) 
                        {
                             //echo "no decimal";
                            $amountdata=intval($amountdata);
               ...
               $flagrespamountcc=1; $pattpaymntammount = "/^\d{2,4}$/"; // $pattpaymntammount = "/^\d{1,4}$/"; 
                         ...
                         if($amountdata<50)
                                    {
                                        ...
                                        //$validation_cc_err_msg="<p>Amount is lesser than  $1</p>";
                                    }
            ...
            if($allowccpaymntflag==true)
            {
                ...
                if(!empty($callchargecreditresp) && ( $callchargecreditresp['flagresp']==1 ))
                {
                    ...
                    $dataorderInsert['create_date_tz']=$servertimezn;
                ...
                //*** update user_master table starts ******************************
                        ...
                        // There is an additional 2% Credit Card transaction fee on purchases.  $2 has been added to your wallet."
                            $notetext = "There is an additional ".$add_perc_amt."% Credit Card transaction fee on purchases. ".$currency_icon.$amountdata." has been added to your Wallet. ";
               ...
               //*** update user_master table ends *********************************
                ...
                if (fmod($tot_wallet_amount, 1) == 0) 
                        {
                             //echo "no decimal";
                            $tot_wallet_amount=intval($tot_wallet_amount);
    ...
    public function showtransactionlist(Request $request)
    {
                ...
                if ($request->session()->has('front_id_sess'))
                {
                 ...
                 $local_tz_data=  $request->input('local_tz_data','') ;
        
             $selectfields=" id,payment_for,charge_token,refund_resp_token,payment_description,payment_status,
                 total_price,payment_scheme,debitorcredit,gigmaster_id,invoice_num,user_id,create_date,create_date_tz,                 
        ...
        //****** for pagination ends*************************************************
         ...
         //*** get view data content starts*************************
            ...
            $data['local_tz_data']=$local_tz_data;
            ...
            if( !empty($total_refunded_credit))
            {
                $total_debit=$total_debit+$total_refunded_credit;
            }
        
             $total_debit=number_format(round(floatval($total_debit),2),2,'.',''); //** now wallet
            ...
            //***** modify figure to show starts ***************************
        
             if (fmod($total_credit, 1) == 0) 
                        {
                             //echo "no decimal";
            ...
            if (fmod($now_wallet_balance, 1) == 0) 
                        {
                             //echo "no decimal";
                            $now_wallet_balance=intval($now_wallet_balance);
                            
                        }
        
        
                       
            //***** modify figure to show ends *****************************
            ...
            //$price = trim($request->input('price'));
              ...
              //***** now**************************************************************************************
                ...
                if($current_wallet_amount>0)
                {
                 ...
                 if (fmod($deduct_amt_from_wallet, 1) == 0) 
                        {
                             //echo "no decimal";
                            $deduct_amt_from_wallet=intval($deduct_amt_from_wallet);
...
public function makecreditcardpaymentrefund(Request $request)
{
    //****** function to make credit card payment refund starts *********
                ...
                $user_id=0; $message='';
                 ...
                 $amount_refunded=0; $tot_wallet_amount=0; $total_credit=0; $total_debit=0; $now_wallet_balance=0;
                $refund_status_data=0;
                ...
                $loggedinusernickname='';
                    if(!empty($user_id))
                   {
                        $fetchtype='single'; $tablename="user_master";
                        //$fieldnames=" first_name,middle_name,last_name,email,wallet_amount,currency ";
                  $fieldnames="address1,country,state,city,zip,first_name,middle_name,last_name,email,wallet_amount,currency,nickname"; 
                        $wherear=array();
                        $wherear['id']=$user_id;
                        $orderbyfield="id"; $orderbytype="asc";
                        $limitstart=0;$limitend=0;                
                        ...
                        if(!empty($fetchuserdata))
                        {
                             ...
                             $current_wallet_amount=$fetchuserdata->wallet_amount;
                             $currency_country_id=$fetchuserdata->currency; // country id of the country 
                             $loggedinusernickname = stripslashes($fetchuserdata->nickname);
                    ...
                    if($current_wallet_amount >= $total_price)
                    {
                                if(!empty($charge_token))
                            {
                                ...
                                */
                                ...
                                if(!empty($chk_ref))
                                {
                                    if( ($chk_ref['flagresp']==1) || ($chk_ref['flagresp']==2) )
                                    {
                                    ...
                                    elseif($chk_ref['flagresp']==3)
                                    {
                                        $message_resp=$chk_ref['message'];
                                        $message="Charge token related to this cannot be processed for insufficient fund in pin account";
                                        $flagresp_output=0;
            ...
            if($flagresp_output==1)
            {
                ...
                $updateAr['refund_status']=$refund_status_data; 
                $updateAr['modified_date']=$date_data;
                
                if($refund_status_data==1) // if refunded
                {
            ...
            //*********** for modified  total_credit ends ****************
                ...
                //**** send mail if required starts *******************
                  ...
                  $sendrefundemail = $this->refundsuccessemail($loggedin_user_email,$loggedinusernickname,$amount_refunded);
    ...
    //******************* email send for refund  starts here
   public function refundsuccessemail($loggedin_user_email,$loggedinusernickname,$amount_refunded)
      {
                    ...
                    $replacefrom =array('{USER}','{AMOUNT}','{SITENAME}','{YEAR}','{BASE_URL}',"{LOGO_IMG}");
                    $replaceto =array(ucfirst($loggedinusernickname),'$ '.$amount_refunded,$sitename,$copyright_year,$bsurl,$logoIMG);
                    
                    mailsnd($Temid=55,$replacefrom,$replaceto,$loggedin_user_email);
                  //*********Helper Function Ends here 
                
                     
      }
    //******************* email send for refund ends here
