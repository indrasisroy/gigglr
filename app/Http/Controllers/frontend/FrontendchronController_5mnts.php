...
class FrontendchronController extends Controller
{
     ...
     public function releaseescrowpayment(Request $request)
    {
         ...
         //******* fetch data from settings table ends ********
            ...
            $artist_qry=" SELECT
            ggm.`id`,ggm.`giguniqueid`,ggm.`total_amount`,ggm.`artist_id`,ggm.`type_flag`,ggm.`booker_id`,ggm.`booking_status`,ggm.`dispute_flag`,ggm.`payment_flag`,ggm.`event_type`,ggm.`booking_cancellation_fee`,ggm.`artist_security_deposit`,now( ) as just_now_datetime ,ggm.`event_end_date_time`, (time_to_sec(timediff(now( ), `event_end_date_time`)) / 60 ) as hours_event_passed
    ...
    public function cancelgigevent(Request $request)
      ...
      //***********Email function for refferer starts here
         ...
         public function refereeremail($artistname,$email,$ref_com_creditedtorerrer_wallet,$gigid)
      {
            //echo "receiver_name=>".$receiver_name;die;
                   ...
                   // *********Helper Function Starts here
                    ...
                    $replaceto =array($artistname,$ref_com_creditedtorerrer_wallet,$sitename,$copyright_year,$bsurl,$logoIMG,$gigid);
...
//*********************************************************************************************************
    ...
    public function releaseofsecuritymoneytoagvwallet(Request $request)
    {
        ...
        $qryforsecuritymoney = "SELECT
            ggm.`id`,ggm.`giguniqueid`,ggm.`artist_id`,ggm.`type_flag`,ggm.`booking_status`,ggm.`dispute_flag`,ggm.`payment_flag`,ggm.`event_type`,ggm.`booking_cancellation_fee`,ggm.`artist_security_deposit`,now( ) as just_now_datetime ,ggm.`booking_accept_date`, (time_to_sec(timediff(now( ), `booking_accept_date`)) / 60 ) as hours_event_passed
...
public function eventnotifytouseremail(Request $request)
{
        ...
        $qryfornotifyuser = "SELECT
            ...
            ,now( ) as just_now_datetime ,ggm.`event_start_date_time`, (time_to_sec(timediff( `event_start_date_time`, now( ))) / 3600 ) as hours_event_remains
