    ...
    public function getReviewData(Request $request)
    {
          ...
          $FromBookerReviewquery = "SELECT rrvd . * ,rrvv.category_name,rrvv.genre_name, um.nickname AS showname,um.seo_name,um.city, um.address1, um.address2, IF( ISNULL( uimg.image_name )  , 'noimage', uimg.image_name ) AS image_name
               ...
               if(!empty($unionmerge))
              {
                ...
                // print_r($review_data);
                // die;
                  $data['user_testi']=$review_data;
                  $data['artistusr_id'] = $user_id;
                  $data['atrist_sessionID'] = $sessnID;
               //  $data['mainserch_qry_data']=$mainserch_qry_lim_data;
                 $view_obj = View::make('front.user.profilereviewajax',$data);
     ...
     public function commonn_email($receiver_email,$receiver_name)
      {
       ...
       public function booking_email_artist($artistvenuegroupfirstname,$artistvenuegrouplastname,$artistvenuegroupusrname,$artistvenuegroupemail,$bookerusrname,$bookingcategory_name,$bookinggenre_name,$address1val,$address2val,$towndata,$statename,$zipdata,$countryname,$bkngdate,$bkngexpirdate,$expirtim)
      {
            //echo "receiver_name=>".$receiver_name;die;
           ...
           //   die;
              ...
              public function chkduplicacy($booking_datedata,$start_timedata,$artist_ID,$sess_id,$end_timedata)
            {
                                       ...
                                       $ttt= explode("/",$booking_datedata);
                                        ...
                                        $endbookingdatetime = $bkngdate." ".$enddatetime;
