    ...
    public function mainsearch(Request $request)
    {
           ...
           //**** if user is logged in then get addr_lat and addr_long  for center point for radius search ends
            ...
            if(!empty($locationtxt))
            {
                       ...
                       // echo "latlog=><pre>"; print_r($latlog); echo "</pre>";
                        
                        $latlng_flag=0;
                        $latlng_flag=$latlog['flag'];
                        $latitude='';$longitude='';
                       
                       if(!empty($latlng_flag) && ($latlng_flag==1))
                       {
            ...
            //***** query for fetching starts
                ...
                $user_master_qry=" ( ";
                        ...
                        Left JOIN (
                         ...
                         select count(rrd.artistgroupvenue_id) as totlrevnum, sum(rrd.point_perc_indrev) tot_perc_data ,
                        round(sum(rrd.point_perc_indrev)/(count(rrd.artistgroupvenue_id))) as point_perc ,
                        rrd.*  from (
                        SELECT `gigmaster_id`,`punctuality`,`performance`,`artistgroupvenue_id`,
                        `punctuality`+`performance`+`presentation` as  total_points,
                        round((( punctuality + performance+presentation)/15)*100) as point_perc_indrev,
                        `booker_flag_type`   FROM `gig_review` WHERE `artistgroupvenue_flag_type`=1
                        ) as rrd

                        group by rrd.artistgroupvenue_id
                        
                        
                        
                        ) as  ggrvw
                        
                        on usm.id= ggrvw.artistgroupvenue_id
                        ...
                        {
                                    $srch_dt_time=$available_date.' '.$available_time;
                                    ...
                                    WHERE type_flag=1 and booking_status=1  and 
                                    
                                     '".$srch_dt_time."' BETWEEN  date_format(event_start_date_time,'%Y-%m-%d %H:%i:%s') AND date_format(event_end_date_time,'%Y-%m-%d %H:%i:%s')
                                    ...
                                    and usm.id NOT IN (
                                            ...
                                            SELECT DISTINCT usmgrp.um_id
                                            FROM (

                                            SELECT DISTINCT um.`id` AS um_id, ggm.`id` AS ggm_id, ggm.`booking_status`, ggm.`event_start_date_time` , ggm.`event_end_date_time`,
                                            ggm.`type_flag` as ggm_type_flag,ggm.`artist_id`  as ggm_artist_id
                                            FROM `user_master` AS um
                                            INNER JOIN group_master AS grm ON grm.`creater_id` = um.`id`
                                            INNER JOIN gig_master AS ggm ON ggm.`artist_id` = grm.`id` and ggm.`type_flag`=2

                                            WHERE ggm.`booking_status` =1


                                            AND '".$srch_dt_time."' BETWEEN  date_format(event_start_date_time,'%Y-%m-%d %H:%i:%s') AND date_format(event_end_date_time,'%Y-%m-%d %H:%i:%s')
                                    ...
                                    and usm.id NOT IN (
                                                ...
                                                SELECT DISTINCT usmvn.um_id
                                                FROM (

                                                SELECT DISTINCT um.`id` AS um_id, ggm.`id` AS ggm_id, ggm.`booking_status`, ggm.`event_start_date_time` , ggm.`event_end_date_time`,
                                                ggm.`type_flag` as ggm_type_flag,ggm.`artist_id`  as ggm_artist_id
                                                FROM `user_master` AS um
                                                INNER JOIN venue_master AS vm ON vm.`creater_id` = um.`id`
                                                INNER JOIN gig_master AS ggm ON ggm.`artist_id` = vm.`id` and ggm.`type_flag`=3
                        ...
                        elseif(!empty($available_date) )
                        {
                                    ...
                                    group_concat(event_start_time separator '||') as eventstarttime, group_concat(event_start_date_time  separator '||' )  as eventstartdatetime ,
                                    group_concat(booking_status separator'||') as booking_status , group_concat(id   separator '||' ) as gigmasterid FROM `gig_master`
                                    WHERE type_flag=1 and booking_status=1  and ( date_format(event_start_date_time,'%Y-%m-%d') = '".$available_date."' or date_format(event_end_date_time,'%Y-%m-%d') = '".$available_date."'  )
                                    group by artist_id
                                     ...
                                     and usm.id NOT IN (
                                          ...
                                          and ( date_format(event_start_date_time,'%Y-%m-%d') = '".$available_date."' or date_format(event_end_date_time,'%Y-%m-%d') = '".$available_date."'  )
                ...
                //**** for group_master   starts
                 ...
                 $user_group_qry.=" select gm.id,gm.type_flag,IF(gm.nickname='', '---', gm.nickname) as profile_name,
                   ...
                   $user_group_qry.=" JOIN  (  SELECT gpsk.group_id, gpsk.skill_id as category_id, gpsk.name as category_name ,
                        ...
                        on gm.id= ggrvw.artistgroupvenue_id
                  ...
                  $user_group_qry.=" where gm.nickname!='' and gm.status=1  and gm.id IN ( SELECT gm.`id` FROM `group_master` as gm
                                    INNER JOIN `user_master` as  um on um.`id`= gm.`creater_id` where um.status=1 )   ";                 
             ...
             if(!empty($available_date) && (!empty($available_time)))
                        {
                                    ...
                                    select gigm.usergrpvenid from (
                                    
                                    SELECT artist_id as usergrpvenid,type_flag,group_concat( date_format(event_date,'%Y-%m-%d') separator '||') as eventstartdate,
                                    group_concat(event_start_time separator '||') as eventstarttime, group_concat(event_start_date_time  separator '||' )  as eventstartdatetime ,
                                    group_concat(booking_status separator'||') as booking_status , group_concat(id   separator '||' ) as gigmasterid FROM `gig_master`
                                    WHERE type_flag=2 and booking_status=1  AND
                                    ...
                                    and gm.id NOT IN (
                                        ...
                                        SELECT DISTINCT grcrgig.grm_id
                                        FROM (

                                        SELECT DISTINCT grm.`id` AS grm_id, grm.`creater_id` , ggm.`id` AS ggm_id, ggm.`booking_status`  , ggm.`event_start_date_time` , ggm.`event_end_date_time`
                                        FROM `group_master` AS grm
                                        INNER JOIN gig_master AS ggm ON ggm.`artist_id` = grm.`creater_id` and ggm.type_flag=1
                                        WHERE ggm.`booking_status` =1
                                     ...
                                     and gm.id NOT IN (
                                        ...
                                        SELECT DISTINCT grcrgigven.grm_id
                                        FROM (

                                        SELECT DISTINCT grm.`id` AS grm_id, grm.`creater_id` as grm_creater_id ,vnm.`id` as vnm_id,vnm.`creater_id` as vnm_creater_id , ggm.`id` AS ggm_id, ggm.`booking_status`, ggm.`event_start_date_time` , ggm.`event_end_date_time`,
                                        ggm.`type_flag` as ggm_type_flag,ggm.`artist_id`  as ggm_artist_id
                                        FROM `group_master` AS grm
                                        INNER JOIN venue_master AS vnm ON vnm.`creater_id` = grm.`creater_id`
                                        INNER JOIN gig_master AS ggm ON ggm.`artist_id` = vnm.`id` and ggm.`type_flag`=3
                         ...
                         elseif(!empty($available_date) )
                        {
                                    ...
                                    group_concat(event_start_time separator '||') as eventstarttime, group_concat(event_start_date_time  separator '||' )  as eventstartdatetime ,
                                    group_concat(booking_status separator'||') as booking_status , group_concat(id   separator '||' ) as gigmasterid FROM `gig_master`
                                    WHERE type_flag=2 and booking_status=1  and ( date_format(event_start_date_time,'%Y-%m-%d') = '".$available_date."' or date_format(event_end_date_time,'%Y-%m-%d') = '".$available_date."'  )
                                    group by artist_id
                                    ) as gigm
            ...
            if($curr_lat!='' && $curr_long!='')
            {
                ...
                $user_venue_qry.=" JOIN  (
                        ...
                        on vm.id= ggrvw.artistgroupvenue_id
                         
                         ";
                         
                 
                                    
               $user_venue_qry.="  where vm.nickname!='' and vm.status=1  and vm.id IN ( SELECT vm.`id` FROM `venue_master` as vm
                                    INNER JOIN `user_master` as  um on um.`id`= vm.`creater_id` where um.status=1 ) ";
             ...
             if(!empty($available_date) && (!empty($available_time)))
                        {
                                    ...
                                    group_concat(event_start_time separator '||') as eventstarttime, group_concat(event_start_date_time  separator '||' )  as eventstartdatetime ,
                                    group_concat(booking_status separator'||') as booking_status , group_concat(id   separator '||' ) as gigmasterid FROM `gig_master`
                                    WHERE type_flag=2 and booking_status=1  
                                    ...
                                    and vm.id NOT IN (
                                        ...
                                        SELECT DISTINCT vncrgig.vnm_id
                                        FROM (

                                        SELECT DISTINCT vnm.`id` AS vnm_id, vnm.`creater_id` , ggm.`id` AS ggm_id, ggm.`booking_status`  , ggm.`event_start_date_time` , ggm.`event_end_date_time`
                                        FROM `venue_master` AS vnm
                                        INNER JOIN gig_master AS ggm ON ggm.`artist_id` = vnm.`creater_id` and ggm.type_flag=1
                                        WHERE ggm.`booking_status` =1
                                     ...
                                     and vm.id NOT IN (
                                        ...
                                        SELECT DISTINCT vncrgiggrp.vnm_id
                                        FROM (

                                        SELECT DISTINCT vnm.`id` AS vnm_id, vnm.`creater_id` as vnm_creater_id , ggm.`id` AS ggm_id, ggm.`booking_status`, ggm.`event_start_date_time` , ggm.`event_end_date_time`,
                                        ggm.`type_flag` as ggm_type_flag,ggm.`artist_id`  as ggm_artist_id
                                        FROM `venue_master` AS vnm
                                        INNER JOIN group_master AS grm ON grm.`creater_id` = vnm.`creater_id`
                                        INNER JOIN gig_master AS ggm ON ggm.`artist_id` = grm.`id` and ggm.`type_flag`=2
...
//            $body = $main_srch_union; //email body
//        
//            $body.= "<br> ************ user_master_qry=> ".$user_master_qry ."********* <br>";
//            $body.= "<br> ************ user_group_qry=> ".$user_group_qry ."********* <br>";
//            $body.= "<br> ************ user_venue_qry=> ".$user_venue_qry ."********* <br>";
//        
//            
       ...
       // echo "=main_srch_union_lim=>$$$$$ ".$main_srch_union_lim." $$$$$"; 
        ...
        //****** for pagination ends*************************************************
            ...
            if(!empty($searchgenrecategdataAr))
            {
                        ...
                        if(in_array($mainsearch,$filtercategnameAr))
                        {
                                    $categnmkeypos=array_search($mainsearch,$filtercategnameAr);
                        ...
                        if( in_array($mainsearch,$filtergenreAr) )
                        {
                                    $genrenmkeypos=array_search($mainsearch,$filtergenreAr);
