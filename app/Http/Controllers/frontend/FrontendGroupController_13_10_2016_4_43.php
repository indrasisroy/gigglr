               ...
               $userstesti = DB::table('gig_review as grv')
                 ...
                 $data['user_testi']=$userstesti;//usrIMG
                  ...
                  // added for category and genre end //
                    ...
                    $group_currency = DB::select( DB::raw("SELECT urm.currency FROM `group_master` as grp, `user_master`as urm WHERE grp.`id`='".$user_id."' and grp.`creater_id`= urm.`id`"));
          ...
          public function checksaveusername($request,$id=0)
           {
                ...
                "seo_name" => "required|min:5|max:50|unique:group_master,nickname,".$id
                ],[
     ...
     function groupprofilesubmit(Request $request){
                    ...
                    //********************checking group active or deactive********************//
                       ...
                       if($artist_id_checking->status=='1')
                       {
                                        ...
                                        $replacefrom =array('{add1}','{add2}','{City}','{Zip}','{State}','{Counrty}','{User}','{WHO}','{SKILL}','{SUBSKILL}','{WHEN}','{SITENAME}','{YEAR}','{BASE_URL}',"{LOGO_IMG}");
                                        $replaceto =array(ucfirst($address1val),ucfirst($address2val),$city,$zipdata,$state_name,$country_name,ucfirst($sender_name),ucfirst($booker_name),$Emailskill,$Emailsubskill,$EmailWhen,$sitename,$copyright_year,$bsurl,$logoIMG);
