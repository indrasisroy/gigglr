            ...
            //***** query for fetching starts
                ...
                $user_master_qry=" ( ";
                         ...
                         if(!empty($available_date) && (!empty($available_time)))
                        {
                                    ...
                                    WHERE type_flag=1 and booking_status=1  and date_format(event_date,'%Y-%m-%d') = '".$available_date."' and ( event_start_time >='".$available_time."' and event_end_time <='".$available_time."' )
                                    group by artist_id
             ...
             if(!empty($available_date) && (!empty($available_time)))
                        {
                                    ...
                                    group_concat(event_start_time separator '||') as eventstarttime, group_concat(event_start_date_time  separator '||' )  as eventstartdatetime ,
                                    group_concat(booking_status separator'||') as booking_status , group_concat(id   separator '||' ) as gigmasterid FROM `gig_master`
                                    WHERE type_flag=2 and booking_status=1  and date_format(event_date,'%Y-%m-%d') = '".$available_date."' and ( event_start_time >='".$available_time."' and event_end_time <='".$available_time."' )
                                    group by artist_id
                                    ) as gigm
