...
//     echo " negotiation_id_type_flag => ".$negotiation_id_type_flag."==".$session_id." w_am_i ".$w_am_i.' $w_am_i_id '.$w_am_i_id;die;
    ...
    //   $isInserted = DB::table('gig_bidrequest')->insert($Insert_into_gig_bidrequest);
    //   $rerurnArray['flag'] = 1; 
    //   $rerurnArray['msg'] = "You have responded to your Booking Request"; 
    // }else{
     ...
     //echo "Hello booking by booker and artist ";
                ...
                if($check_alreadyexceptedval!='' && $check_alreadyexceptedval>0)
                {
                 ...
                 //*********  Here we are going to complete the accept process from both end by release escrow amount
                  ...
                  //***********  if escrow amount is released then update the table starts here
                    ...
                    $updatedatatogigbidreqarray['gig_description'] = $gig_description;
      ...
      }else
      {
          // echo "Here we are first time entering the data into gig bid request table";die;
            ...
            $insertdatatogigbidreqarray['gig_description'] = $gig_description;
    ...
    function accptbidrequestbybooker_test(Request $request){
     ...
     $gig_master_details = DB::table('gig_master')->select('id','giguniqueid','event_timezone','event_start_date_time','event_address_lat','event_address_long','booking_status')->where('id',$gigmaster_id)->first();
     ...
     if($event_start_date_time_in_seconds > $maxrichedtime_in_seconds){
      ...
      //$data['booking_status']= "Possible";
           ...
           //******************Perial payment code end****************//
           
           //******************Full payment code start date 2-10-16****************//
           $amount_trans = $total_payment;
           $gig_master_updae['payment_flag']= '2';
           //******************Full payment code end******************//
           ...
           if($isInserted == 1 && $chkupd == 1){
            ...
            //*********send to artist**********//
              $replacefrom =array('{ARTIST}','{ENVID}','{BOOKER}','{SITENAME}','{YEAR}','{BASE_URL}',"{LOGO_IMG}","{AMOUNT}");
              $replaceto =array(ucfirst($full_artist_name),$event_giguniqueid,ucfirst($full_booker_name),$sitename,$copyright_year,$bsurl,$logoIMG,$amount_trans);
              mailsnd($Temid=24,$replacefrom,$replaceto,$full_artist_email);
                        
            //*********send to booker**********//
              $replacefrom =array('{BOOKER}','{ENVID}','{SITENAME}','{YEAR}','{BASE_URL}',"{LOGO_IMG}");
              $replaceto =array(ucfirst($full_booker_name),$event_giguniqueid,$sitename,$copyright_year,$bsurl,$logoIMG);
           ...
           if($isgig_bidrequestUpdated == 1 && $isgig_masterUpdated == 1 ){
              $rerurnArray['flag'] = 1; 
              $rerurnArray['msg'] = "Bid Accepted successfully.";
              $rerurnArray['booker_curent_balance'] = $booker_curent_balance;
              $replacefrom =array('{ACCEPT_BY}','{User}','{SITENAME}','{YEAR}','{BASE_URL}',"{LOGO_IMG}");
              $replaceto =array(ucfirst($full_artist_name),ucfirst($full_booker_name),$sitename,$copyright_year,$bsurl,$logoIMG);
              mailsnd($Temid=17,$replacefrom,$replaceto,$full_booker_email);

              $replacefrom1 =array('{ACCEPT_BY}','{User}','{SITENAME}','{YEAR}','{BASE_URL}',"{LOGO_IMG}","{Accountfname}");
              $replaceto1 =array(ucfirst($full_booker_name),ucfirst($full_artist_name),$sitename,$copyright_year,$bsurl,$logoIMG,ucfirst($full_artist_name));
              mailsnd($Temid=21,$replacefrom1,$replaceto1,$full_artist_email);
              
           }else{
              $rerurnArray['flag'] = 0; 
              $rerurnArray['msg'] = "Hmm, something went wrong in the event acceptance process";
           }
     }else{
      ...
      if($isgig_bidrequestUpdated == 1 && $isgig_masterUpdated == 1 ){
         ...
         $replacefrom1 =array('{REASON}','{User}','{BOOKER}','{SITENAME}','{YEAR}','{BASE_URL}',"{LOGO_IMG}");
...
//**************add in 16-12-16 start ************//
      ...
      if(!empty($gig_master_details)){
       ...
       if(!empty($artist_mail_details)){
        ...
        //************* Send mail to artist***********//
        $replacefrom =array('{USER}','{BY_WHOM}','{EVENT_NAME}','{SITENAME}','{YEAR}','{BASE_URL}',"{LOGO_IMG}");
        $replaceto =array(ucfirst($artist_name)," by ".$booker_name." as a Booker",$gig_master_details->giguniqueid,$sitename,$copyright_year,$bsurl,$logoIMG);
        mailsnd($Temid=50,$replacefrom,$replaceto,$artist_email);
        //************* Send mail to artist ***********//
        
        //************* Send mail to booker ***********//
        $replacefrom =array('{USER}','{BY_WHOM}','{EVENT_NAME}','{SITENAME}','{YEAR}','{BASE_URL}',"{LOGO_IMG}");
        $replaceto =array(ucfirst($booker_name)," by you ",$gig_master_details->giguniqueid,$sitename,$copyright_year,$bsurl,$logoIMG);
    ...
    function gigcancelbybooker_test(Request $request){
...
//**************add in 16-12-16 start ************//
    ...
    function gigcancelbyartist_test(Request $request){
      ...
      if(!empty($gig_master_details)){
       ...
       if(!empty($artist_mail_details)){
        ...
        //************* Send mail to booker***********//
        $replacefrom =array('{USER}','{BY_WHOM}','{EVENT_NAME}','{SITENAME}','{YEAR}','{BASE_URL}',"{LOGO_IMG}");
        $replaceto =array(ucfirst($booker_name)," by ".$artist_name." as a Artist",$gig_master_details->giguniqueid,$sitename,$copyright_year,$bsurl,$logoIMG);
        mailsnd($Temid=50,$replacefrom,$replaceto,$booker_email);
        //************* Send mail to booker ***********//
        
        //************* Send mail to artist ***********//
        $replacefrom =array('{USER}','{BY_WHOM}','{EVENT_NAME}','{SITENAME}','{YEAR}','{BASE_URL}',"{LOGO_IMG}");
        $replaceto =array(ucfirst($artist_name)," by you ",$gig_master_details->giguniqueid,$sitename,$copyright_year,$bsurl,$logoIMG);
        mailsnd($Temid=50,$replacefrom,$replaceto,$artist_email);
        //************* Send mail to artist ***********//
