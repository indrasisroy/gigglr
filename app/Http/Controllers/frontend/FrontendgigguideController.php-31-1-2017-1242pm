...
public function gigleftpannel(Request $request){
        ...
        $genreQueryImplode[] = "(`type_flag` = '2' and (".implode(" or ",$genreQueryGroup)."))";
            ...
            $genreQueryStr = implode(" or ",$genreQueryImplode);

            $genrefullQuery = "SELECT gigm.*,gskl.`category`,gskl.`genre`,'genre_only' as return_type FROM `gig_master` as gigm,`gig_skill_rel` as gskl WHERE gigm.`id` = gskl.`gigmaster_id` and gigm.`gigpostrequestflag`='1' and gigm.`booking_status` = '1' ".$string_sql." and gigm.`event_type`!='2' and ".$user_city." and gigm.`id` in(SELECT gigmaster_id FROM `gig_skill_rel` WHERE ".$genreQueryStr.")";
        ...
        $favoriterecord_event_sql = "SELECT gigm.*,gskl.`category`,gskl.`genre`,'favorite_only' as return_type FROM `gig_master` as gigm ,`gig_skill_rel` as gskl WHERE gigm.`id` = gskl.`gigmaster_id` and gigm.`gigpostrequestflag`='1' and gigm.`booking_status` = '1' and gigm.`event_type`!='2' and ".$user_city." and (".$afetUnionStr.") ".$string_sql."";
