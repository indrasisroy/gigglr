...
class FrontendreviewController extends Controller
{
     ...
     public function getreviewdata(Request $request)
    {
           ...
           $local_tz=$request->input('local_tz','');
       ...
       if(!empty($user_id))
       {
          ...
          IF( ISNULL( uimg.image_name )  , 'noimage', uimg.image_name ) AS image_name
               FROM (
                   SELECT `id` AS review_date_id, `gigmaster_id`,`booker_id` AS from_id, `booker_flag_type` AS usertype, `punctuality` AS rev_param_one, `performance` AS rev_param_two,`presentation` AS rev_param_three, `agv_review_data` AS review_description, `agv_review_date` AS review_date,`agv_review_date_tz` AS review_date_tz, IF( (
               id >0
           ...
           $FromBookerReviewquery .="  
              ...
              SELECT gigrev.`id` AS review_date_id,`gigmaster_id`,gigrev.`artistgroupvenue_id` AS from_id, gigrev.artistgroupvenue_flag_type AS usertype,`bkr_hospitality` AS rev_param_one,`bkr_environment` AS rev_param_two, `bkr_readiness` AS rev_param_three, `bkr_review_data` AS review_description, `bkr_review_date` AS review_date,`bkr_review_date_tz` AS review_date_tz,           
               IF( (
              gigrev.id >0
