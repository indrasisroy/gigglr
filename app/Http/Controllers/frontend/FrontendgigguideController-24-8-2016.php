...
public function index(Request $request)
 ...
 public function getgigguidecallendardata(Request $request)
    ...
    public function gigleftpannel(Request $request){
        ...
        $user_details_sql = "SELECT uskl.skill_id,uskl.skill_sub_id,us.city FROM `user_skill_rel` as uskl,`user_master` as us WHERE us.`id` = '40' and uskl.`catag_type_id`='1' and us.`id` = uskl.`user_id`";
        ...
        if(!empty($user_details_data)){
            ...
            foreach($user_details_data as $details_data){
                $genreQuery[] = "(`category`='".$details_data->skill_id."' and `genre` ='".$details_data->skill_sub_id."' and `type_flag` ='1')";
            }
            
        }
        $selected_category = trim($request->input('category'));
        $selected_genre = trim($request->input('genre'));
        //$genreQueryStr = implode(" or ",$genreQuery);
        if($gencat == '0'){
            $genreQueryStr = implode(" or ",$genreQuery);
        }else{
            $genreQueryStr = "(`category`='".$selected_category."' and `genre` ='".$selected_genre."')";
        }
        $genrefullQuery = "SELECT *,'genre_only' as return_type FROM `gig_master` WHERE `gigpostrequestflag`='1' and `booking_status` = '1' ".$string_sql." and `event_type`!='2' and `id` in(SELECT gigmaster_id FROM `gig_skill_rel` WHERE ".$genreQueryStr.")";
        //$fainal_quesy[] = $genrefullQuery;
        //********************** genre only end *************//
        
        
        //********************** favoriterecord started *************//
        $user_city = $user_details_data[0]->city;
        $showmytown_event_sql = "SELECT gigm.*,'city_only' as return_type FROM `gig_master` as gigm WHERE gigm.`gigpostrequestflag`='1' and gigm.`booking_status` = '1' and gigm.`event_type`!='2' and gigm.`event_city` = '".$user_city."' ".$string_sql."";
        ...
        $afetUnionStr = implode(" or ",$arraySql);
        $favoriterecord_event_sql = "SELECT gigm.*,'favorite_only' as return_type FROM `gig_master` as gigm WHERE gigm.`gigpostrequestflag`='1' and gigm.`booking_status` = '1' and gigm.`event_type`!='2' and (".$afetUnionStr.") ".$string_sql."";
        ...
        $fainal_quesyStr_event_data=DB::select($fainal_quesyStr);
        $data  = array();
        if(!empty($fainal_quesyStr_event_data)){
            ...
            foreach($fainal_quesyStr_event_data as $fainal_quesyStr_data){
                ...
                $tableName = '';
                if($fainal_quesyStr_data->event_type == '2'){
                ...
                //$data['artist_id'][$i] = $fainal_quesyStr_data->artist_id." ".$tableName;
                //$data['tableName'][$i] = $tableName;
                if($nik!=''){
