	...
	public function contactsupportsubfunc(Request $request)
	{
		...
		//{
			...
			if($chkvalid===true)
			{
				...
				$replacefrom =array('{LOGO_IMG}','{BASE_URL}','{USERREALNAME}','{CONTACTREASON}','{CONTACTMESSAGE}','{REQUESTRESPONSE}','{SENDCOPY}','{SITENAME}','{YEAR}');
				$replaceto =array($logoIMG,$bsurl,$confromname,$contactreasonmain,$contactmessage,$requestresponse,$sendcopy,$sitename,$copyright_year);
				...
				$user_master_db=$user_master_db->where('et.id', '35');
				...
				if(!empty($user_master_db))
				{
					...
					$passarr['adminfrom']=$conemail;
					$passarr['emailsub']=$user_master_db->subject;
					$passarr['emailto']=$contactemail;
					$passarr['sitename']=$sitename;
					$passarr['emailfromname']=$confromname;
					
					$data = array('replacefrom'=>$replacefrom,'replaceto'=>$replaceto,'email_body'=>$body);
