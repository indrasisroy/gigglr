...
class FrontendGroupController extends Controller
{
    ...
    public function getgenere(Request $request)
    {
            ...
            if($categoryId > 0)
            {
                        ...
                        //->where('status','1')
                        //->get();
                        $sqlGroupSubSkill="SELECT * FROM `skill_master` where `id` in(SELECT `skill_sub_id` FROM `group_skill_rel` WHERE `group_id`='".$groupId."' AND `skill_id`='".$categoryId."')";
   ...
   //************* Common function to get single data ends here
      ...
      public function booking_email()
      {
                    ...
                    $replaceto =array(ucfirst(stripslashes($first_name)),$email,$nw_pass,$sitename,$copyright_year,$bsurl,$logoIMG);
                ...
                $inserGroup_Array = array(
                 ...
                 'zip' =>$GroupName[0]->zip
                    ...
                    DB::table('group_master')
                                      ...
                                      ->update(['seo_name' => $seo_name,'nickname' => $seo_name]);
                    ...
                    $userssel = DB::table('settings')
                                        ...
                                        $replacefrom =array('{Accountfname}','{SITENAME}','{YEAR}','{BASE_URL}',"{LOGO_IMG}");
                                        $replaceto =array(stripslashes($sendingMailUser),$sitename,$copyright_year,$bsurl,$logoIMG);
                    ...
                    return redirect('editgroup/'.$seo_name);
                }
        }else{
                    return redirect('/');
        }
          ...
          public function checksaveusername($request,$id=0)
           {
                ...
                "seo_name" => "required|min:5|max:50"
                  ...
                  // 'seo_name.required' => '* Group name is required',
                   'seo_name.required'=> '* A profile title or group name is required',
     ...
     function groupprofilesubmit(Request $request){
                    ...
                    //********************checking group active or deactive********************//
                       ...
                       if($artist_id_checking->status=='1')
                       {
                                        ...
                                        $evntdtcnv= convertdatetothistz($dttm=$pretimezn,$ftmzn=$final_timezone,$ttmzn=$servertimezn,$cnvrtdtdrmt='Y-m-d H:i:s');
                                        ...
                                        $posttimezn = $endtimedt." ".$endtim;
                                        $evntdtendcnv= convertdatetothistz($dttm=$posttimezn,$ftmzn=$final_timezone,$ttmzn=$servertimezn,$cnvrtdtdrmt='Y-m-d H:i:s');
...
//************ Get event ID ends here
                                        ...
                                        $replacefrom =array('{Accountfname}','{Accountlname}','{ARGRPVNU}','{PROFILEALIAS}','{add1}','{add2}','{City}','{Zip}','{State}','{Counrty}','{User}','{WHO}','{SKILL}','{SUBSKILL}','{WHEN}','{SITENAME}','{YEAR}','{BASE_URL}',"{LOGO_IMG}","{ExpiryDate}","{ExpiryTime}","{EVENTID}");
                                        $replaceto =array(ucfirst(stripslashes($artistvenuegroupfirstname)),ucfirst(stripslashes($artistvenuegrouplastname)),'Group',$profilealias,ucfirst(stripslashes($address1val)),ucfirst(stripslashes($address2val)),$city,$zipdata,$state_name,$country_name,ucfirst($sender_name),ucfirst(stripslashes($booker_name)),$Emailskill,$Emailsubskill,$EmailWhen,$sitename,$copyright_year,$bsurl,$logoIMG,$bkngexpirdate,$expirtim,$evntid);
                                        ...
                                        $replaceto =array(ucfirst(stripslashes($bookerdetail[0]->first_name)),ucfirst(stripslashes($booker_name)),$sitename,$copyright_year,$bsurl,$logoIMG,$evntid);
                                        ...
                                        $returnArray['message'] = 'Congratulations your request was sent';
                                        }else{
                       ...
                       $returnArray['flagdata'] = '02';
                       $returnArray['message'] = 'Sorry, but that does not look like a legitimate address';
                    }
                    //print_r($latlog);die;
            ...
            public function chkduplicacy($booking_datedata,$endtimedt,$start_timedata,$artist_ID,$sess_id,$end_timedata)
            {
             ...
             //  print_r($match_date_qry);
              // die();
                                              ...
                                              if ($count_bookingavailable > 0)
                                              {
                                              // $errorMsg[]="Your booking request can not be processed at this moment<br> You have already booked this Group <br> Please select another date time to book this Group";
                                              }
     ...
     //*************** 01-07-end ******************
            ...
            //****************************************  split booking validation starts ******************

             public function checkduplicatedatafuncgroup(Request $request)
            {
              


               $respAr =array();$venueqrycount=0; $artistqrycount=0;$groupqrycount=0;$groupcreater = '';$venueid='';
              ...
              //******************************* overlapping checking starts here 
...
//                        print_r($rqry_venue3);
                    ...
                    }  //*******  end of ($venueid!='' && $venueid > 0)
                
                
                
            } //**********  end of ($groupcreater!='' && $groupcreater > 0)
            ...
            if($venueqrycount > 0 || $artistqrycount > 0 || $groupqrycount > 0)
              ...
              //******************************* overlapping checking ends here


            }  //**************  end of main function  


            public function groupbookingaddresschk(Request $request)
            {
