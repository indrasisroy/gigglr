...
{       
     ...
     public function reply(Request $request)
        ...
        public function artistbookertransactionbyadmin(Request $request){
                ...
                $pay_to_artist_id_gv = $request->input('pay_to_artist_id');
            ...
            //****** fetch site and referral commision ********//
            ...
            //**** start fetching data for wallet update and sending mail body content ***//
                $todayis=date("d/m/Y");
                
                if($pay_to_booker_id_value!='' && $pay_to_booker_id_value>0)
                {
                    $booker_partamount="$".$pay_to_booker_id_value;
                }else{
                    $booker_partamount='no amount';
                }
                
                if($pay_to_artist_id_value!='' && $pay_to_artist_id_value>0)
                {
                    $artist_partamount="$".$pay_to_artist_id_value;
                }else{
                    $artist_partamount='no amount';
                }
                ...
                if(!empty($gigalldata))
                {
                    ...
                    if($event_typeflag=='1')
                    {
                        $artdetails = DB::table('user_master')->select('nickname','email','wallet_amount')->where('id', $event_artistid)->first();
                        ...
                        if(count($artdetails) > 0)
                        {
                            ...
                            $artist_wallet=$artdetails->wallet_amount;
                            $artist_email=$artdetails->email;
                            $pay_to_artist_id = $event_artistid;
                        }
                        else{
                    ...
                    elseif($event_typeflag=='2')
                    {
                        $artdetails=DB::select("SELECT grp.nickname AS groupnickname, grp.creater_id AS grpcrid, usms1.nickname AS grpcreatorname, usms1.email AS grpcreatoremail, usms1.wallet_amount AS grpartistWallet FROM group_master AS grp JOIN user_master AS usms1 ON usms1.id=grp.creater_id WHERE grp.id='".$event_artistid."'");
                        ...
                        if(count($artdetails) > 0)
                        {
                            ...
                            $artist_wallet=$artdetails[0]->grpartistWallet;
                            $artist_email=$artdetails[0]->grpcreatoremail;
                            $pay_to_artist_id = $artdetails[0]->grpcrid;
                        }
                        else{
                            $event_artist_nickname='';
                            $event_artist_emailid='';
                    ...
                    elseif($event_typeflag=='3')
                    {
                        $artdetails=DB::select("SELECT ven.nickname AS venuenickname, ven.creater_id AS vencrid, usms2.nickname AS vencreatorname, usms2.email AS vencreatoremail, usms2.wallet_amount AS venartistWallet FROM venue_master AS ven JOIN user_master AS usms2 ON usms2.id=ven.creater_id WHERE ven.id='".$event_artistid."'");
                        ...
                        if(count($artdetails) > 0)
                        {
                            ...
                            $artist_wallet=$artdetails[0]->venartistWallet;
                            $artist_email=$artdetails[0]->vencreatoremail;
                            $pay_to_artist_id = $artdetails[0]->vencrid;
                        }
                        else{
                            $event_artist_nickname='';
                            $event_artist_emailid='';
            ...
            //**** end fetching data for wallet update and sending mail body content ***//
            ...
            //*************artist start*************//
                ...
                //***** start artist main wallet and site commission calculation *****//
                    $dataamount = '';
                    $sitedataamount='';
                    if($srmdetails!=''){
                       ...
                       $sitedataamount=(($pay_to_artist_id_value + $srmdetails)*$site_commission)/100;
                    }else{
                        $dataamount = $pay_to_artist_id_value - (($pay_to_artist_id_value*$site_commission)/100);
                        $artist_wallet_new_wallet = $artist_wallet + $dataamount;
                        //$artist_wallet_new_wallet = $artist_wallet+(($pay_to_artist_id_value*(100-$site_commission))/100);
                        $sitedataamount=($pay_to_artist_id_value*$site_commission)/100;
                    }
                    ...
                    $respdataAr['site_commission_amout_from_artist'] = $sitedataamount;
                
                    //************* insert into user_order for artist credit and debit **************//
                        $artist_wallet_add_cr = 'true';
                        $artist_wallet_add_dr = 'true';
                        if($pay_to_artist_id_value!='0'){
                            $trnstype11 = 'C';
                            $artist_wallet_add_cr = $this->insertintouserorder($pay_to_artist_id,$artist_email,$pay_to_artist_id_value,$gig_id,$trnstype11,'DPR','Dispute payment release');
                        
                            $trnstype22 = 'D';
                            $artist_wallet_add_dr =  $this->insertintouserorder($pay_to_artist_id,$artist_email,$sitedataamount,$gig_id,$trnstype22,'SCP','Site Commission payment');
                            
                            //*** inserting site commission amount credited to site into user_order table
                                $today_recent_dt=date('Y-m-d H:i:s');
                                $logggedin_userIP = get_client_ip_server();
                                $site_wallet_add=array();
                                $site_wallet_add['payment_for']="SCC";
                                $site_wallet_add['payment_description']='Commission amount credited to site';
                                $site_wallet_add['email']='';
                                $site_wallet_add['total_price']=$sitedataamount; 
                                $site_wallet_add['user_ip_address']=$logggedin_userIP;
                                $site_wallet_add['debitorcredit']="C"; // C=> Credit , D=> Debit
                                $site_wallet_add['gigmaster_id']=$gig_id;
                                $site_wallet_add['invoice_num']="INV-".time().rand(111111,999999);
                                $site_wallet_add['user_id']=0;
                                $site_wallet_add['create_date']=$today_recent_dt;
                                $site_wallet_add['modified_date']=$today_recent_dt;
                                $site_wallet_add['payment_status']='SUCCESS';
                               
                                $isInsertedSC = DB::table('user_order')->insert($site_wallet_add);
    
                                $isInsertedSC=DB::getPdo()->lastInsertId();
                            
                            //**-----mail sending to the site
                            $site_partamount="$".$sitedataamount;
                ...
                //***** end artist main wallet and site commission calculation *****//
                
                //*** start referral id checking and payment calculation ***//
                    $recentdt=date('Y-m-d');
                    $cherefer_mail = "SELECT um . * FROM  `user_master` AS um,  `referral_email` AS ref_eml WHERE ref_eml.`referrer_userid` = um.`id` AND ref_eml.`user_id` =  '".$pay_to_artist_id."' AND um.`status` =  '1' AND DATE( ref_eml.`referral_expiry_date` ) >= '".$recentdt."' ";
                    $cherefer_mail_result = DB::select( DB::raw($cherefer_mail));
                
                    if(!empty($cherefer_mail_result)){
                        //********** refer commission calculation ****************//
                        ...
                        //**** refer related upadte into user_master table ****//
                        ...
                        //********** referral commission insert into user_order table ****************//
                        if($datainsrtrefer){
                            $trnstype33 = 'C';
                            $artist_refer_wallet_add_dr =  $this->insertintouserorder($refer_id,$refer_mail,$refer_amout_from_artist,$gig_id,$trnstype33,'RC','Referal Credit');
                        ...
                        $respdataAr['refer_amout_from_artist'] = '';
                    }
                //*** end referral id checking and payment calculation ***//
                }
                else{
                    $respdataAr['artist_wallet_new_wallet'] = $artist_wallet;
                    $respdataAr['artist_wallet_new_amout'] = $pay_to_artist_id_value;
                    $respdataAr['site_commission_amout_from_artist'] = '';
                    $respdataAr['refer_amout_from_artist'] = '';
                }
            ...
            if($pay_to_booker_id_value!='' && $pay_to_booker_id_value>0)
            {
                ...
                //*************insert into user_order**************//
                    ...
                    if($pay_to_booker_id_value!='0'){
                            $booker_wallet_add_cr = $this->insertintouserorder($pay_to_booker_id,$booker_email,$pay_to_booker_id_value,$gig_id,$trnstype44,'DPR','Dispute payment release');    
                    }
                ...
                $respdataAr['booker_wallet_new_wallet'] = $booker_wallet;
                $respdataAr['booker_wallet_new_amout'] = $pay_to_booker_id_value;
            }
            //*************booker end*************//
                
            //**** resolving dispute and status update for both dispute and gig_master tables ******//    
