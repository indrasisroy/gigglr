...
class AdminuserController extends Controller
{
            ...
            //if($chkvalid!=1)
            {
                ...
                if(empty($uid))
                {
                  ...
                  //  $lastInserted=DB::getPdo()->lastInsertId();
                    $isInserted=1;
                    if($isInserted){
                        $mailvalid=$this->sendEmailNewuser($email,$username,$pwd); //*** Sending mail after inserting an user ***//
                    }
                }
                else
                {
    ...
    public function sendEmailNewuser($email,$username,$pwd)
    {
        $passarr=array();
        ...
        $subject = "New account in Prosessional.com";
        ...
        $data = array(
                      'username'=>$username,'password'=>$pwd,'sitename'=>$sitename,'copyright_year'=>$copyright_year
                      );
    
        Mail::send('admin.emailviewfolder.customemailtenplate', $data, function ($message) use ($passarr) {
