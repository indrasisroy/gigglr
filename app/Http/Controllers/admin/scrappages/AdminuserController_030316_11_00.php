...
class AdminuserController extends Controller
{
    ...
    public function index(Request $request )
    {
        ...
        $users_db=$users_db->select(DB::raw('um.id,CONCAT_WS(" ",um.first_name,um.last_name) AS name,um.username,um.email,um.password,um.status'));
            ...
            //if($chkvalid!=1)
            {
                ...
                if(empty($uid))
                {
                    ...
                    if($isInserted){
                        ...
                        ->where('id', 1)
                        ->get();
                        $from=$userssel[0]->email;
                        $to = $email;
                        $subject = "New account in Prosessional.com";
                        
                        //$mailvalid=$this->sendEmailReminder($request,$lastInserted);
                    }
                }
                else
                {
