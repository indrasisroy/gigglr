        ...
        */
        ...
        {     
                ...
                $homesearch_db = DB::table('home_search as hs')
                	     ->join('skill_master','hs.skill_id','=','skill_master.id')
                             ->select(DB::raw('hs.id,hs.title,skill_master.name,hs.image_name,hs.image_title,hs.status'));
                //$homesearch_db=$homesearch_db->where('hs.status',1);
                
                if(!empty($srch1))
                {
                        $homesearch_db=$homesearch_db->where('hs.title', 'like', "%".$srch1."%")
                                                      ->orWhere('skill_master.name', 'like', "%".$srch1."%");
                }
        ...
        public function addhomesearch(Request $request,$id=0)
        {
                ...
                //****** genre ar *****
                $home_genreidarr=array();
                $home_genreidarr['']="Select a Genre";
                ...
                if(!empty($id))
                {
                        ...
                        */ 
                        ...
                        if(!empty($homesearchrow))
                        {
                            ...
                            $sskl_db = DB::table('skill_master as sm');
                            $sskl_db=$sskl_db->select(DB::raw('sm.id,sm.name'));
                            $sskl_db=$sskl_db->where('sm.status', '=', 1);
                            $sskl_db=$sskl_db->where('sm.parent_id', '=',$homesearch_skillid);
                            $sskl_db=$sskl_db->orderBy('sm.name', 'asc');
                            $sskl_db= $sskl_db->get();

                            if(!empty($sskl_db))
                            {
                                foreach($sskl_db as $sskl_obj)
                                {
                                    $home_genreidarr[$sskl_obj->id]=stripslashes($sskl_obj->name);
                                }       
                            }
                ...
                $data['home_genreidarr']=$home_genreidarr;
        ...
        public function savehomesearch(Request $request)
        {
                ...
                $skill_sub_id = addslashes(trim($request->input('skill_sub_id',0)));
                ...
                $dataInsert['skill_sub_id']=$skill_sub_id;
        ...
        public function checkhomesearchform($request,$id=0)
        {
                ...
                $validator = Validator::make($request->all(), [
                      ...
                      'homesearch_skillid.required' => "* Category field is required",
                      //  'skill_sub_id.required' => "* Genre field is required",
        ...
        public function delhomesearch(Request $request,$id=0)
        {
         ...
         public function populategenre(Request $request,$id=0)
        {
                ...
                $sskl_db = DB::table('skill_master as sm');
                            $sskl_db=$sskl_db->select(DB::raw('sm.id,sm.name'));
                            $sskl_db=$sskl_db->where('sm.status', '=', 1);
                            $sskl_db=$sskl_db->where('sm.parent_id', '=',$skill_id);
                            $sskl_db=$sskl_db->orderBy('sm.name', 'asc');
                            $sskl_db= $sskl_db->get();
                            ...
                            if(!empty($sskl_db))
                            {
                                foreach($sskl_db as $sskl_obj)
                                {
                                    ...
                                    $respAr[]=array('subskill_id'=>$sskl_obj->id,'subskill_name'=>stripslashes($sskl_obj->name));
                                }       
                            }
