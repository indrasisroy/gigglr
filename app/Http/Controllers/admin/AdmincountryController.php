    ...
    public function checkcountryform($request,$id=0)
        ...
        public function delcountry(Request $request,$id=0)
       {
        ...
        $at='';
           ...
           // }else if($i>1){
                ...
                foreach ($id_arry as $value) {
                    $chkdelcountry = $this->deletecheck($value);
                    //echo " ->".$value;
                   if($chkdelcountry !=1)
                  {
          ...
          // }
           ...
           if($ar>0 && $at>0 && $chkdelcountry !=1)
           {
            
                 $request->session()->flash('admin_successmsgdata_sess', 'Country and corresponding states are delete successfully.');  
           }else if($ar>0 && $at>0 && $chkdelcountry ==1)
           {
            
            $request->session()->flash('admin_successmsgdata_sess', 'Country and corresponding states are delete successfully. But some country data can not be deleted');
           }
           else if($ar=='' && $at=='' && $chkdelcountry ==1)
           {
            
            $request->session()->flash('admin_errormsgdata_sess', 'Country and corresponding states can not be deleted');
           }
           // echo "Case 4=== ar =".$ar.'at='.$at.'  chkdelcountry'.$chkdelcountry;die;
           else
           {
             $request->session()->flash('admin_successmsgdata_sess', 'Country and corresponding states are delete successfully.'); 
           }
        ...
        public function deletecheck($id)
           {
            $deletecheckstatususr =  DB::table('user_master')->where('country',$id)->first();
            $deletecheckstatusgrp =  DB::table('group_master')->where('country',$id)->first();
            $deletecheckstatusvenue =  DB::table('venue_master')->where('country',$id)->first();
