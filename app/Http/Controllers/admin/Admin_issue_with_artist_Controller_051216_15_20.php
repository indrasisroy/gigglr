     ...
     public function reply(Request $request)
        {
               ...
               //-----------Createing datainsert arrey for insert into resolve_dispute_replies table--------//
                ...
                //-----------condition for email template data-------------//
                ...
                if($booker_ID != '' && $artist_ID==''){
                        ...
                        {       
                                ...
                                $sendMail =  mailsnd($Temid=31,$replacefrom,$replaceto,$artist_ori_email);
                                        ...
                                        $sendMail =  mailsnd($Temid=44,$replacefrom,$replaceto,$booker_ori_email);
                        }
                                //-----END------------------------------//
        ...
        public function artistbookertransactionbyadmin(Request $request){
                ...
                $gigalldata = DB::table('gig_master')->select(DB::raw('event_start_date_time','event_city','type_flag','artist_id','booker_id'))->where('id', $gig_id)->first();
                ...
                if(!empty($gigalldata))
                {
                    ...
                    $bkrdetails = DB::table('user_master')->select('nickname')->where('id', $event_bookerid)->first();
                    ...
                    if($event_typeflag==1)
                    {
                        $artdetails = DB::table('user_master')->select('nickname')->where('id', $event_artistid)->first();
                        if(count($artdetails) > 0)
                        {
                                $event_artist_nickname = $artdetails->nickname;
                    ...
                    elseif($event_typeflag==2)
                    {
                        $artdetails=DB::select("SELECT grp.nickname AS groupnickname, grp.creater_id, usms1.nickname AS grpcreatorname FROM group_master AS grp JOIN user_master AS usms1 ON usms1.id=grp.creater_id WHERE grp.id='".$event_artistid."'");
                        //$artdetails = DB::table('group_master')->select('nickname')->where('id', $event_artistid)->first();
                        if(count($artdetails) > 0)
                        {
                                $event_artist_nickname = $artdetails->grpcreatorname;    
                                $event_mainartist_nickname=$artdetails->groupnickname;
                    ...
                    elseif($event_typeflag==3)
                    {
                        $artdetails=DB::select("SELECT ven.nickname AS venuenickname, ven.creater_id, usms2.nickname AS vencreatorname FROM venue_master AS ven JOIN user_master AS usms2 ON usms2.id=ven.creater_id WHERE ven.id='".$event_artistid."'");
                        //$artdetails = DB::table('venue_master')->select('nickname')->where('id', $event_artistid)->first();
                        if(count($artdetails) > 0)
                        {
                                $event_artist_nickname = $artdetails->vencreatorname;
                                $event_mainartist_nickname=$artdetails->venuenickname;
                                $self='in '.$event_mainartist_nickname.' group';
                                $self1='in your '.$event_mainartist_nickname.' group';
                        }
                        else{
                                $event_artist_nickname='';
                                $event_mainartist_nickname='';
                ...
                //********mail sending to booker and artist of resolved details********//
                  ...
                  //------mail to booker-----//
                        ...
                        //$sendMail =  mailsnd($Temid=38,$replacefrom,$replaceto,$booker_ori_email);
                  //---------end---------//
                  ...
                  //------mail to artist-----//
                        ...
                        //$sendMail =  mailsnd($Temid=37,$replacefrom,$replaceto,$booker_ori_email);
                  //---------end---------//
