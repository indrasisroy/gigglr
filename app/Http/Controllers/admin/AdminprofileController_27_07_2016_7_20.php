      ...
      if($validate_admin_data === true)
      {
        ...
        $nw_pass = md5($password);
         ...
         $updateemail = $this->sendEmailNewuser($first_name,$emailaddress,$password,$username); //email sending code sttarts here
          ...
          $request->session()->flash('admin_successmsgdata_sess', 'Profile successfully updated');
    ...
    public function sendEmailNewuser($to_name,$email,$pwd,$usernamelogin)
    {
        ...
        $sendingpassword = 'Password :'.$pwd;
                ...
                //**********Email Send Code Starts here
                  ...
                  $replaceto =array(ucfirst($to_name),$usernamelogin,'',$sitename,$copyright_year,$bsurl,$logoIMG);
                  }
                  else
                  {
                  $replacefrom =array('{NAME}','{USERNAME}','{PASSWORD}','{SITENAME}','{YEAR}','{BASE_URL}',"{LOGO_IMG}");
                  $replaceto =array(ucfirst($to_name),$usernamelogin,$sendingpassword,$sitename,$copyright_year,$bsurl,$logoIMG);
                  }
                $email_r = mailsnd($Temid=2,$replacefrom,$replaceto,$email);
