...
class AdminsupportbypageController extends Controller
{
        ...
        **/
        ...
        public function index(Request $request )
        {
                ...
                $howitsdone_db=$howitsdone_db->select(DB::raw('supportbypage.id,supportbypage.title,supportbypage.description,supportbypage.create_date,supportbypage.modified_date,supportbypage.status,supportbypage.order_id'));
                if(!empty($srch1))
                {
        ...
        public function savesupportbypage(Request $request)
        {
            ...
            //echo "========== <br>";
                ...
                $dataInsert=array();
                $nxtorderid=''; $fetchmxid=''; $fetchmx='';
                if($id == 0)
                {
                        $dataInsert['status']=1;
                        
                         $fetchmx=DB::select("SELECT MAX(order_id) AS mxorderid FROM help_supportbypage");
                         $fetchmxid=$fetchmx[0]->mxorderid;
                         $nxtorderid=$fetchmxid+1;
                         
                         $dataInsert['order_id']=$nxtorderid;
                }
        ...
        //public function orderingchangesupportbypage(Request $request)
        //{       
        //        $orderchange = $request->input('orderid');
        //        $mainid = $request->input('bymainid');
        //        
        //        $respAr=array();
        //        $flagdata=0;
        //        
        ...
        //                    
        //                    $dataUpdate=array();
        //                    $dataUpdate['order_id']=$orderchange;
        //                    
        //                    $updstaus=DB::table('help_supportbypage')
        //                          ->where('id', $mainid)
        //                          ->update($dataUpdate);
        ...
        public function orderingchangesupportbypage(Request $request){
            ...
            $datasave = $request->input('datasave',0);
            $supid =    $request->input('dataid',0);
            
            $respAr=array();
            //$sup_db = DB::table("help_supportbypage");
            //$sup_db=$sup_db->where('order_id',$datasave);
            //$sup_db=$sup_db->first();
            
            //if(!empty($faq_db)){
            //    $respAr['flag']=0;
            //    $respAr['iddata']="Sorry! You can't able to update this order";
            //}else{
                $updateAr=array();
                $updateAr['modified_date']=date('Y-m-d H:i:s');
                $updateAr['order_id']=$datasave;
                
                $chkupd= DB::table('help_supportbypage')->where('id',$supid)->update($updateAr);
                
                if($chkupd){
                    $respAr['flag']=1;
                    $respAr['changedata']=$datasave;
                    $respAr['iddata']="Ordering data saved successfully";  
                }else{
                    $respAr['flag']=0;
                    $respAr['changedata']='';
                    $respAr['iddata']="Something wrong";
                }
            //}
