...
class AdminfaqController extends Controller
{
        ...
        **/
        ...
        public function index(Request $request )
        {
                ...
                $faq_db=$faq_db->select(DB::raw('fc.id,fc.title,fc.description,fc.publish,fc.create_date,fc.modified_date,fc.order_id'));
                if(!empty($srch1))
                {
             ...
             //echo "========== <br>";
                ...
                if($chkvalid===true)
                {
                        ...
                        if(empty($id))
                        {
                                ...
                                $fetchmx=DB::select("SELECT MAX(order_id) AS mxorderid FROM faq");
        ...
        public function faqordersave(Request $request){
            
            $datasave = $request->input('datasave',0);
            $faqid =    $request->input('dataid',0);
            
            $respAr=array();
            $faq_db = DB::table("faq");
            $faq_db=$faq_db->where('order_id',$datasave);
            $faq_db=$faq_db->first();
            ...
            //}else{
                ...
                $chkupd= DB::table('faq')->where('id',$faqid)->update($updateAr);
                
                if($chkupd){
                    $respAr['flag']=1;
                    $respAr['changedata']=$datasave;
                    $respAr['iddata']="You have successfully updated";  
                }else{
                    $respAr['flag']=0;
                    $respAr['changedata']='';
      ...
      //*********Custom SEO Function ends here
       ...
       //********************  delete multiple faq starts here *************
        ...
        public function faqdeletemultiple(Request $request,$id=0)
        {
