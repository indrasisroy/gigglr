...
class AdminskillController extends Controller
{
    ...
    public function index(Request $request )
    {
     // echo "I am inside skill management";exit();
        ...
        $main_qry_srchsklid=0;
        $srchsklid_main_arr = array();
           ...
           // $skills_db=$skills_db->select(DB::raw('ls.id,(CASE WHEN ls.parent_id = 0 THEN true ELSE ls.parent_id END) AS is_user,ls.name,ls.status,lu.typename'));
            ...
            if(!empty($srch1))
            {
                $main_qry_srchsklid = DB::select("SELECT sklmst.id FROM skill_master AS sklmst WHERE sklmst.name LIKE '%".$srch1."%' AND sklmst.parent_id='0'");
                if(count($main_qry_srchsklid)>0){
                     foreach($main_qry_srchsklid as $qry_srchsklid){
                         $srchsklid=$qry_srchsklid->id;
                         array_push($srchsklid_main_arr,$srchsklid);
                     }
                }
                
                $skills_db=$skills_db->whereIn('ls.parent_id', $srchsklid_main_arr);
                
                $skills_db=$skills_db->orWhere('ls.name', 'like', "%".$srch1."%");   
            }
    ...
    public function addskill(Request $request,$id=0)
    {
        ...
        if(!empty($requestpost_sess) && array_key_exists("catag_type",$requestpost_sess))
        {
         ...
         $uesrskilltypeidAr['']="Select genre";
          if(!empty($id))
          {
                     //**** fetch data starts
    ...
    public function saveskill(Request $request)
    {
            ...
            //if(!empty($id))
            //{
            //    $skill_master = DB::table('skill_master as ls');
            //    $skill_master = $skill_master->select(DB::raw('ls.parent_id'));
            //    $skill_master = $skill_master->where('id', '=', $id);
            //    $skill_master = $skill_master->get();
            //    $parent_id=$skill_master[0]->parent_id;
            //    //echo "hi====================".$parent_id;die;
            //}
            //else{
                $parent_id = addslashes(trim($request->input('parent_id','0')));
                //echo "open====================".$parent_id;die;
            //}
           ...
           if($chkvalid===true)
           {
                  if(empty($id))
                  {
                        ...
                        //******
                            ...
                            $seoname_val = $seoname_val.'--';
                            //********** Checking SEO name exists or not strats here

                            $seoqry = DB::table('skill_master');
                            $seoqry=$seoqry->where('seo_name', 'like', $seoname_val.'%');
                            

                            $seoqry=$seoqry->count();
                            $tot_seocount=$seoqry;
                            $tot_seocount = $tot_seocount+1;
                            $seoname_val = $seoname_val.$tot_seocount;
                  ...
                  if($isInserted >= 0 )
                  {
                         ...
                         $request->session()->flash('admin_successmsgdata_sess', 'Genre Successfully saved.');
                         return redirect(ADMINSEPARATOR.'/skill');
        ...
        public function checkskillsform($request,$id=0,$parntsklname)
            {
                    ...
                    */
                     ...
                     $validator = Validator::make($request->all(), [
                        'catag_type' => "required",    
                        'name' => "required|skillunq_name:".$id.",".$parent_id.",".$parntsklname.",".$catag_type."|unique:skill_master,name,".$id.",id,catag_type,".$catag_type.",parent_id,".$parent_id
                    ],[
                       'catag_type.required'=>'*User field required', 
                       'name.required'=>'*Genre name field required',
                       'name.skillunq_name'=>'*This genre name already exists for its parent',
                    ]);
            ...
            public function checkskillsform2nd($request,$id=0,$parntsklname)
            {
                ...
                $validator = Validator::make($request->all(), [   
                    ...
                    'name.required'=>'* Genre name field required',
                    'name.alpha_spaces'=>'*This genre name only contain letters and spaces',
                    
                ]);
                
                //|alpha_spaces|unique:skill_master,name,".$id
                ...
                //{
                    ...
                    $validator->after(function($validator)  use ($userData)  {         
                        ...
                        //$sklname = $request->input('name','');
                        //if(empty($sklname!=''))
                        //{
                        //if(empty($id))
                        //{
                            $validatenamechk=$this->chknameduplicacy($request);
                            if (!empty($validatenamechk))
                            {
                                $validator->errors()->add('name', $validatenamechk);
                            }
                        //    if(empty($id))
                        //{
                            $validateutypechk=$this->chkutypereq($request);
                            if (!empty($validateutypechk))
                            {
                                $validator->errors()->add('catag_type', $validateutypechk);
                            }
                        //}
                        //}
                        //}
        ...
        public function chkutypereq($request)
        {
        ...
        public function chknameduplicacy($request)
        {
            $sklname = addslashes(trim( $request->input('name')));
            $parent_id2 = addslashes(trim($request->input('parent_id')));
            $id = addslashes(trim($request->input('skillid',0)));
            //echo "first==========".$parent_id;die;
            ...
            if(empty($id))
            {
                ...
                {
                    $skill_master = DB::table('skill_master as ls');
                    $skill_master = $skill_master->select(DB::raw('ls.id'));
                    $skill_master = $skill_master->where('parent_id', '=', $parent_id2);
                    $skill_master = $skill_master->where('name', '=', $sklname);
                    $skill_master = $skill_master->get();
                    $tot2 = count($skill_master);
      
                    if($tot2>0)
                    {
                        $errorMsg[]="* This genre already exists into this parent category";
                    }
                }
                else{
                    $skill_master = DB::table('skill_master as ls');
                    $skill_master = $skill_master->select(DB::raw('ls.id'));
                    $skill_master = $skill_master->where('parent_id', '=', 0);
                    $skill_master = $skill_master->where('name', '=', $sklname);
                    $skill_master = $skill_master->get();
                    $tot2 = count($skill_master);
                    ...
                    if($tot2>0)
                    {
                        $errorMsg[]="* This genre name already exists as a parent category";
                    }
                }
            }
            else{
                ...
                $skill_master_ftch = DB::table('skill_master as ls');
                $skill_master_ftch = $skill_master_ftch->select(DB::raw('ls.parent_id'));
                $skill_master_ftch = $skill_master_ftch->where('id', '=', $id);
                $skill_master_ftch = $skill_master_ftch->get();
                $parent_id1=$skill_master_ftch[0]->parent_id;
                ...
                if($parent_id1==0)
                {
                    ...
                    if($tot2>0)
                    {
                        $ftchdskillid=$skill_master[0]->id;
                        if($ftchdskillid!=$id)
                        {
                            $errorMsg[]="* This genre name already exists as a parent category";
                        }
                    ...
                    $skill_master = $skill_master->select(DB::raw('ls.id'));
                    $skill_master = $skill_master->where('name', '=', $sklname);
                    $skill_master = $skill_master->where('parent_id', '=', $parent_id1);
                    $skill_master = $skill_master->get();
                    $tot2 = count($skill_master);
      
                    if($tot2>0)
                    {
                        //$ftchdparentid=$skill_master[0]->parent_id;
                        //if($ftchdparentid==$parent_id)
                        $ftchdskillid=$skill_master[0]->id;
                        if($ftchdskillid!=$id)
                        {
                            $errorMsg[]="* This genre already exists into this parent category";
                ...
                //$skill_master = DB::table('skill_master as ls');
                //$skill_master = $skill_master->select(DB::raw('ls.id,ls.parent_id'));
                //$skill_master = $skill_master->where('name', '=', $sklname);
                //$skill_master = $skill_master->get();
                //$tot2 = count($skill_master);
                //
                ...
                //    if($ftchdskillid!=$id)
                //    {
                //        //echo "last============".$parent_id;die;
                //        if(empty($parent_id))
                //        {
                //            $errorMsg[]="* This skill name already exists as a parent skill category";
                //        }
                //        else{
                //            $ftchdparentid=$skill_master[0]->parent_id;
                //            if($ftchdparentid==$parent_id)
                //            {
                //                $errorMsg[]="* This skill already exists into this parent skill category";
                //            }
                //        }
                //    }
                //}
        ...
        public function delskill(Request $request,$id=0)
       {
           ...
           $user_skill_rel=$user_skill_rel->orwhere('skill_sub_id', '=', $id);        
           $user_skill_rel=$user_skill_rel->get();

           //echo "<br>From user skill rel======>".
            $tot1=count($user_skill_rel);

           // $user_subskill_rel = DB::table('user_skill_rel');
           // $user_subskill_rel=$user_subskill_rel->where('skill_sub_id', '=', $id);          
           // $user_subskill_rel=$user_subskill_rel->get();


          //echo "<br>From user sub skill rel======>". 
          //$tot2=count($user_subskill_rel);


          //** for group
           $group_skill_rel = DB::table('group_skill_rel');
           $group_skill_rel=$group_skill_rel->where('skill_id', '=', $id); 
           $group_skill_rel=$group_skill_rel->orwhere('skill_sub_id', '=', $id);          
           $group_skill_rel=$group_skill_rel->get();

          // echo "<br>From group skill rel======>".
            $tot3=count($group_skill_rel);

           // $group_subskill_rel = DB::table('group_skill_rel');
           // $group_subskill_rel=$group_subskill_rel->where('skill_sub_id', '=', $id);          
           // $group_subskill_rel=$group_subskill_rel->get();

          // echo "<br>From group sub skill rel======>". 
           //$tot4=count($group_subskill_rel);

           //******* for venue

           $venue_skill_rel = DB::table('venue_skill_rel');
           $venue_skill_rel=$venue_skill_rel->where('skill_id', '=', $id);    
           $venue_skill_rel=$venue_skill_rel->orwhere('skill_sub_id', '=', $id);        
           $venue_skill_rel=$venue_skill_rel->get();

           //echo "<br>From venue skill rel======>". 
           $tot5=count($venue_skill_rel);

           // $venue_subskill_rel = DB::table('venue_skill_rel');
           // $venue_subskill_rel=$venue_subskill_rel->where('skill_sub_id', '=', $id);          
           // $venue_subskill_rel=$venue_subskill_rel->get();

          // echo "<br>From venue sub skill rel======>".
           // $tot6=count($venue_subskill_rel);



            //*******for gig skill rel
                $gig_skill_rel = DB::table('gig_skill_rel');
                $gig_skill_rel=$gig_skill_rel->where('category', '=', $id);   
                $gig_skill_rel=$gig_skill_rel->orWhere('genre', '=', $id);    
                $gig_skill_rel=$gig_skill_rel->get();

           //echo "<br>From venue skill rel======>". 
           $tot7=count($gig_skill_rel);

           
            
            if($tot1>0 || $tot3>0 ||  $tot5>0  ||  $tot7>0)
            {
                $flag1=0;
           ...
           $skill_master=$skill_master->where('parent_id', '=', $id);
            ...
            //echo "<br>From skill_master child any ======>". 
              $tot2=count($skill_master);
          ...
          {
                 // echo "<br>delete block";
                  $ar=DB::table('skill_master')->where('id', '=', $id)->delete();

                  if($ar>0)
                  {
                        $request->session()->flash('admin_successmsgdata_sess', 'Genre data delete successfully.');  
                  }
          }
          else
          {
                  //echo "<br> no delete block";
                  $request->session()->flash('admin_errormsgdata_sess', 'Genre data cannot be  deleted.'); 
          }
             // die;
           return redirect(ADMINSEPARATOR.'/skill');
       }
        ...
        public function commonchangeskill(Request $request)
       {
        ...
        public function gtseoname($id,$name)
        {
         ...
         //******** function for seo name ends here  
           ...
           //*********multiple skill delete starts here
        public function skilldeletemultiple(Request $request,$id=0)
        {
           $i = strlen($id);
           $tot1 = 0;
           $tot3 =0;
...
if($countdeletditems>0 && $countdeletditems == $id_arry_count)
{
  $request->session()->flash('admin_successmsgdata_sess', 'Genre data delete successfully.');
}else if($countdeletditems > 0 && $countdeletditems != $id_arry_count)
{
  $request->session()->flash('admin_successmsgdata_sess', 'Genre data delete successfully. But some genre data could not be deleted.');
}else if($countdeletditems == 0 && $countdeletditems != $id_arry_count)
{
  $request->session()->flash('admin_errormsgdata_sess', 'Genre data cannot be  deleted.'); 
}
          ...
          //**********multiple skill delete endes here
