...
{       
        public function index(Request $request ){
            ...
            if(!empty($srch1))
            {
				...
				if($srch1=='yes' || $srch1=='YES' || $srch1=='Yes')
				{
						$nxtsrchyes=1;
						$nxtsrchno='';
				}
				if($srch1=='no' || $srch1=='NO' || $srch1=='No')
				{
						$nxtsrchno=0;
						$nxtsrchyes='';
				}
            ...
            $countries_db = DB::table('resolve_dispute as rd')
                ...
                ->where(function($query) use ($srch1,$nxtsrchyes,$nxtsrchno){
						//if(!empty($srch1) && ($nxtsrchyes=='' && $nxtsrchno=='')){
                        if(!empty($srch1)){
						...
						//if(!empty($nxtsrchyes))
						//{
						//		$query->where('rd.dispute_resolved_status', '=', "%".$nxtsrchyes."%");		
						//}
						//if(!empty($nxtsrchno))
						//{
						//		$query->where('rd.dispute_resolved_status', '=', "%".$nxtsrchno."%");
						//}
...
//				$countries_db = DB::table('resolve_dispute as rd');
//				$countries_db=$countries_db->Join('user_master as um', function($join)
//												{
//														$join->on('um.id','=','rd.dispute_opener_id');
//												});
//                $countries_db=$countries_db->Join('gig_master as gm', function($join)
//												{
//														$join->on('gm.id','=','rd.gig_id');
//												});
//				$countries_db=$countries_db->select('rd.id','rd.gig_name','rd.dispute_opener_id','rd.gig_id','rd.gig_unique_id','rd.dispute_opening_date','rd.dispute_type','rd.issue_description','rd.arrival','rd.arrival_time','rd.required_specifications_availability','rd.able_to_complete','rd.technical_issue','rd.receive_rider','rd.leave_early','rd.leaving_time','rd.venue_presentation','rd.getting_gig','rd.commencing_gig','gm.artist_id','gm.booker_id','gm.type_flag','um.email','um.nickname','um1.email as booker_email','um1.nickname as booker_nickname','rd.dispute_resolved_status','rd.dispute_resolved_date');
//                $countries_db=$countries_db->Join('user_master as um1', function($join)
//												{
//														$join->on('gm.booker_id','=','um1.id');
//												});
//				$countries_db=$countries_db->where('rd.dispute_type','4');
//				$countries_db=$countries_db->get();
        ...
        public function reply_view(Request $request,$id=0){
          ...
          $countries_db = DB::table('resolve_dispute as rd')
                ...
                ->select('rd.id','rd.gig_name','rd.dispute_opener_id','rd.gig_id','rd.gig_unique_id','rd.dispute_opening_date','rd.dispute_type','rd.issue_description','rd.arrival','rd.arrival_time','rd.required_specifications_availability','rd.able_to_complete','rd.technical_issue','rd.receive_rider','rd.leave_early','rd.leaving_time','rd.venue_presentation','rd.getting_gig','rd.commencing_gig','gm.artist_id','gm.booker_id','gm.total_amount','um.email','um.nickname','um1.email as booker_email','um1.nickname as booker_nickname','rd.dispute_resolved_status','rd.dispute_resolved_date','gm.artist_security_deposit')
		...
		if(!empty($srmdetails)){
           ...
           return view('admin.admin_issue_with_booker.admin_issue_with_booker_reply_view', $data);
           //echo '<pre>';print_r($data);exit;
        }
